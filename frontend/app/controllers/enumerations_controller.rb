class EnumerationsController < ApplicationController
  ...
  def index
    ...
    @enumeration = JSONModel(:enumeration).find(params[:id]) if params[:id] && !params[:id].blank?
  end
  ...
  def update_value
    ...
    begin
      ...
      JSONModel::HTTP.post_form("#{@enumeration_value.uri}/position", position: params[:position]) if params[:position]
  ...
  def create
    ...
    if params[:enumeration].blank? || params[:enumeration][:value].blank?
      flash.now[:error] = "#{I18n.t('enumeration.value')} is required"
