...
def run_tests(opts)
  ...
  (repo_id = r[:body]['id']) || fail('Repository creation', r)
  ...
  (second_repo_id = r[:body]['id']) || fail('Second repository creation', r)
  ...
  (acc_id = r[:body]['id']) || fail('Accession creation', r)

  puts 'Request the accession'
  r = do_get(url("/repositories/#{repo_id}/accessions/#{acc_id}"))

  r[:body]['title'] =~ /integration test accession/ ||
  ...
  r[:body]['id'] || fail('Second accession creation', r)
  ...
  r = do_post({
    ...
    vocabulary: '/vocabularies/1'
              ...
              url('/subjects'),
              'text/json')
  (r[:status] === '400') || fail('Invalid subject check', r)
  ...
  (subject_id = r[:body]['id']) || fail('Subject creation', r)
  ...
  (coll_id = r[:body]['id']) || fail('Resource creation', r)
  ...
  (ao_id = r[:body]['id']) || fail('Archival Object creation', r)
  ...
  r[:body].last['errors'] || fail('Catch reference errors', r)
  ...
  if opts[:check_ldap]
    ...
    (r[:status] == '403') || fail('LDAP login with incorrect password', r)

    r = do_post(URI.encode_www_form(password: ''),
                url('/users/marktriggs/login'))

    (r[:status] == '403') || fail('LDAP login with blank password', r)

    r = do_post(URI.encode_www_form(password: 'testuser'),
                url('/users/marktriggs/login'))

    r[:body]['session'] || fail('LDAP login with correct password', r)

    r = do_get(url(r[:body]['user']['uri']))
    (r[:body]['name'] == 'Mark Triggs') || fail('User attributes from LDAP', r)

    puts 'Check username completion'
    r = do_get(url('/users/complete?query=mark'))
    (r[:body].first == 'marktriggs') || fail('LDAP username completion', r)
  end
  ...
  def state.get_last_mtime(*_args)
    0
  end
  ...
  begin
    (Integer(r[:body]['total_hits']) > 0) || fail('Search indexing', r)
  rescue TypeError
  ...
  begin
    (Integer(r[:body]['total_hits']) == 0) || fail('Repository scoping', r)
  rescue TypeError
    puts "Response: #{r.inspect}"
  end
  ...
  begin
    ...
    (Integer(r[:body]['total_hits']) == 1) || fail('Search within record tree', r)
  rescue TypeError
    puts "Response: #{r.inspect}"
  end
  ...
  (r.code == '303') || (r.code == '300') || fail('fetch by external ID', r)

  puts 'It refuses to delete a non-empty repository'
  r = do_get(url("/repositories/#{repo_id}/groups"))
  (r[:body].count > 0) || fail('Groups should not be gone', r)
  ...
  (r[:body]['code'] == 'SESSION_EXPIRED') || fail('Session expiry', r)
  ...
  (r[:status] == '403') || fail('batch deleting when not authenticated', r)
end
