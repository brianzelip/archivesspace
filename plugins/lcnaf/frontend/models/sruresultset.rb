...
class SRUResultSet
  ...
  def initialize(response_body, query, page, records_per_page)
    ...
    @records = doc.xpath('//recordData/record').map { |record|
      ...
      title << "#{record.xpath("./datafield[@tag='100']/subfield[@code='b']").text.strip} " if record.xpath("./datafield[@tag='100']/subfield[@code='b']")

      title << record.xpath("./datafield[@tag='100']/subfield[@code='c']").inject('') { |_result, node| "#{node.text.strip} " } if record.xpath("./datafield[@tag='100']/subfield[@code='c']")

      title << "#{record.xpath("./datafield[@tag='100']/subfield[@code='q']").text.strip} " if record.xpath("./datafield[@tag='100']/subfield[@code='q']")

      title << "#{record.xpath("./datafield[@tag='100']/subfield[@code='d']").text.strip} " if record.xpath("./datafield[@tag='100']/subfield[@code='d']")
