module AutoGenerator
  ...
  def update_from_json(json, opts = {}, apply_nested_records = true)
    self.class.properties_to_auto_generate.each do |generate_opts|
      next if generate_opts[:only_if] && !generate_opts[:only_if].call(json)

      if generate_opts[:only_on_create]
        # force the value back to the original value from the DB
        json[generate_opts[:property]] = send(generate_opts[:property])
      else
        next if generate_opts[:only_if_nil] && !json[generate_opts[:property]].nil?
  ...
  module ClassMethods
    def create_from_json(json, opts = {})
      ...
      properties_to_auto_generate.each do |generate_opts|
        if (generate_opts[:only_if]&.call(json)) || json[generate_opts[:property]].nil?
          json[generate_opts[:property]] = generate_opts[:generator].call(json)
          modified = true
        end
      end
