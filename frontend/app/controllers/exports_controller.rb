class ExportsController < ApplicationController
  ...
  def download_ead
    url = if params[:print_pdf] == 'true'
            "/repositories/#{JSONModel.repository}/resource_descriptions/#{params[:id]}.pdf"
          else
            "/repositories/#{JSONModel.repository}/resource_descriptions/#{params[:id]}.xml"
          end

    download_export(url,
                    include_unpublished: (params[:include_unpublished] || false),
                    print_pdf: (params[:print_pdf] || false),
                    include_daos: (params[:include_daos] || false),
                    numbered_cs: (params[:numbered_cs] || false),
                    ead3: (params[:ead3] || false))
  end
  ...
  def download_export(request_uri, params = {})
    ...
    respond_to do |format|
      format.html {
        response.headers['Content-Type'] = meta['mimetype'] if meta['mimetype']
        response.headers['Content-Disposition'] = "attachment; filename=#{meta['filename']}"
