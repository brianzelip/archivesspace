class Enumeration < Sequel::Model(:enumeration)
  ...
  def migrate(old_value, new_value)
    old_enum_value = enumeration_value.find { |val| val[:value] == old_value }

    raise AccessDeniedException, "Can't transfer from a read-only enumeration value" if old_enum_value.readonly != 0

    new_enum_value = enumeration_value.find { |val| val[:value] == new_value }

    raise NotFoundException, "Can't find a value '#{new_value}' in enumeration #{id}" if new_enum_value.nil?

    dependants = self.class.dependants_of(name) || []
    ...
    reload
    enumeration_value.each_with_index { |ev, i| ev.position = i; ev.save }
  ...
  def self.apply_values(obj, json, opts = {})
    ...
    obj.reload
    is_editable = ((obj.editable === 1) || (obj.editable == true))
    ...
    # default...
    raise AccessDeniedException, "Cannot modify a non-editable enumeration: #{obj.name} with #{json['values'].join(' , ')}. Only allowed values are : #{obj.enumeration_value.join(' , ')} " if (!is_editable && !added_values.empty?) || (!is_editable && !removed_values.empty?)
    ...
    if EnumerationValue.filter(enumeration_id: obj.id,
                               ...
                               readonly: 1).count > 0
      raise AccessDeniedException, "Can't remove read-only enumeration values"
    end
    ...
    removed_values.each do |value|
      DB.attempt {
        ...
        raise ConflictException, "Can't delete a value that's in use: #{value}"
      }
    end
  ...
  def self.create_from_json(json, opts = {})
    ...
    apply_values(super, json, opts.merge(default_value: default_value))
  end
  ...
  def self.sequel_to_jsonmodel(objs, opts = {})
    ...
    jsons.zip(objs).each do |json, obj|
      ...
      json['default_value'] = EnumerationValue[id: obj.default_value][:value] if obj.default_value
    end
