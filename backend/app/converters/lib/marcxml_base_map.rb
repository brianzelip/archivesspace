module MarcXMLBaseMap
  ...
  def subject_template(getterms, getsrc, _variant_tag = nil)
    {
      ...
      map: {
        ...
        "//datafield[@tag='040']" => lambda { |subject, node|
          if record_properties[:type] == :authority
            subject.source = 'lcgft' if (subject.source == 'Other') && (node.at_xpath("subfield[@code='f']").inner_text == 'lcgft')
          end
        }
  ...
  def make_term(term_type, term)
    { term_type: term_type, term: term, vocabulary: '/vocabularies/1' } if !term.empty? && !term_type.nil?
  end
  ...
  def agent_template
    {
      ...
      map: {
        "subfield[@code='e']" => lambda { |agent, node|
          agent['_relator'] = node.inner_text
        },
        "subfield[@code='4']" => lambda { |agent, node|
        ...
        "//datafield[@tag='046']" => {
          ...
          map: {
            'self::datafield' => proc { |date, node|
              ...
              date.end = end_date if (date.begin && end_date) && (end_date.to_i > date.begin.to_i)
  ...
  def bibliography_note_template(label, template = nil, *tmpl_args)
    {
      ...
      rel: :notes,
      map: {
        'self::datafield' => lambda { |note, node|
  ...
  def sets_primary_and_rest_of_name
    lambda { |name, node|
      ...
      if val.match(/\A(.+),\s*(.+)\s*\Z/)
        name['primary_name'] = Regexp.last_match(1).chomp(',')
        name['rest_of_name'] = Regexp.last_match(2).chomp(',')
      else
  ...
  def subfield_template(template, node, map = nil)
    ...
    while result.match(section)
      if Regexp.last_match(2) == '@'
        val = node.attr("#{Regexp.last_match(3)}")
      else
        val = ''
        node.xpath("subfield[@code='#{Regexp.last_match(3)}']").each do |subnode|
      ...
      val = map&.has_key?(Regexp.last_match(3)) && map[Regexp.last_match(3)].has_key?(val) ? map[Regexp.last_match(3)][val] : val

      if val
        result.sub!(section, "#{Regexp.last_match(1)}#{val}#{Regexp.last_match(4)}")
      else
        result.sub!(section, '')
      end
    end
  ...
  def dates_of_existence_date_for(node, subfields)
    ...
    subfields.each do |sc|
      ...
      date      = date && (date.length >= 4) ? date[0..3] : nil
    end
    date
  end
  ...
  def is_fallback_resource_title
    {
      rel: lambda { |resource, obj|
        resource['_fallback_titles'] ||= []
        resource['_fallback_titles'] << obj.subnotes[0]['content'] if obj.respond_to?(:subnotes)
      }
    }
  end
  ...
  def BASE_RECORD_MAP
    {
      ...
      map: {
        ...
        # CONTROLFIELD
        "//controlfield[@tag='008']" => lambda { |resource, node|
          ...
          if ['i', 'k', 's'].include?(control[6])
            make(:date) do |date|
              ...
              date.begin = control[7..10] if control[7..10]&.match(/^\d{4}$/)

              date.end = control[11..14] if control[11..14]&.match(/^\d{4}$/)
        ...
        # ID_0, ID_1, ID_2, ID_3
        "datafield[@tag='852']" => lambda { |resource, node|
          id = concatenate_subfields(['k', 'h', 'i', 'm'], node, '_')
          resource.id_0 = id unless id.empty?
        },

        "datafield[@tag='090']" => lambda { |resource, node|
        ...
        "datafield[@tag='240']" => mix(multipart_note('odd', 'Uniform Title', '
        ...
        # TITLE
        "datafield[@tag='245']" => lambda { |resource, node|
          resource.title = subfield_template('{$a : }{$b }{[$h] }{$k , }{$n , }{$p , }{$s }{/ $c}', node)

          expression = concatenate_subfields(['f', 'g'], node, '-')
          if expression.empty?
        ...
        "datafield[@tag='255']" => multipart_note('odd', 'Mathematical map data', '
        ...
        "datafield[@tag='260']" => mix(multipart_note('odd', 'Publication Date', '{$c}'),
                                       'self::datafield' => lambda { |resource, node|
        ...
        "datafield[@tag='300']" => {
          ...
          rel: :extents,
          map: {
            'self::datafield' => lambda { |extent, node|
              ex = node.xpath('.//subfield[@code="a"]')
              unless ex.empty?
                ext = ex.first.text
                if ext =~ /^([0-9\.]+)+\s+(.*)$/
                  extent.number = Regexp.last_match(1)
                  extent.extent_type = Regexp.last_match(2)
                end
              end
        ...
        "datafield[@tag='340']" => multipart_note('phystech', 'Physical Medium', '
        ...
        "datafield[@tag='343']" => singlepart_note('physdesc', 'Planar Surface Coordinate System', '
        ...
        "datafield[@tag='352']" => multipart_note('phystech', 'Digital Graphic Representation', '
        ...
        "datafield[@tag='357']" => multipart_note('odd', 'Originator Dissemination Control', '
        ...
        "datafield[@tag='514']" => multipart_note('odd', 'Data quality', '
        ...
        "datafield[@tag='533']" => multipart_note('odd', 'Reproduction Note', '
        ...
        "datafield[@tag='534']" => multipart_note('odd', 'Original Version Note', '
                                            {$p: }{$a, }{$t, }{$k, }{$c }{($b). }{$f. }{$e, }{$m. }{$n, }{$l. }{($x), }{($z)}.'),

        "datafield[@tag='535']" => multipart_note('originalsloc', 'Location of Originals Note', '
                                            {@ind1: } {$3--}{$a. }{$b, }{$c. }{$d }{($g).}',
                                                  'ind1' => { '1' => 'Holder of originals', '2' => 'Holder of duplicates' }),

        # FINDING AID SPONSOR
        "datafield[@tag='536']" => lambda { |resource, node|
          resource.finding_aid_sponsor = subfield_template('
        ...
        "datafield[@tag='541']" => multipart_note('acqinfo', 'Immediate Source of Acquisition', '
        ...
        "datafield[@tag='544']" => multipart_note('relatedmaterial', 'Related Archival Materials', '
        ...
        "datafield[@tag='546']" => lambda { |resource, node|
        ...
        "datafield[@tag='562']" => multipart_note('relatedmaterial', 'Copy and Version Identification', '
        ...
        "datafield[@tag='565']" => singlepart_note('materialspec', 'Case File Characteristics Note', '
        ...
        "datafield[@tag='630' or @tag='130']" => subject_template(
          lambda { |node|
            terms = []
            terms << make_term('uniform_title', concatenate_subfields(['a', 'd', 'e', 'f', 'g', 'h', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't'], node, ' ', true))
        ...
        "datafield[@tag='754']" => subject_template(
          lambda { |node|
            term = concatenate_subfields(['a', 'c', 'd', 'x', 'z'], node, '--')
        ...
        'self::record' => lambda { |resource, _node|
          resource.title = resource['_fallback_titles'].shift if !resource.title && resource['_fallback_titles'] && !resource['_fallback_titles'].empty?

          resource.id_0 = "imported-#{SecureRandom.uuid}" if resource.id_0.nil? || resource.id.empty?
        }
      }
    }
  end
