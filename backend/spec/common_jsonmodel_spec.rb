...
describe 'JSON model' do
  ...
  it 'supports adding custom error handlers' do
    JSONModel.add_error_handler do |error|
      raise NotFoundException, 'Seriously not good enough' if error['code'] == 'OUTTOLUNCH'
    end
    expect {
  ...
  it 'can give a string representation of a validation exception' do
    begin
      ...
      expect(e.to_s).to match /^\#<:ValidationException: /
    end
  end
    ...
    threads << Thread.new do
      1000.times do
        begin
          ...
          (e.errors.keys == ['title']) || raise("Oops: #{e.inspect}")
        end
      end
