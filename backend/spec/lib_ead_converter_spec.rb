...
describe 'EAD converter' do
  ...
  describe 'EAD Import Mappings' do
    ...
    it 'creates the archival object tree correctly' do
      ...
      @archival_objects.each do |k, v|
        if k.to_i > 1
          parent_key = format '%02d', k.to_i - 1
    ...
    it "maps '<odd>' correctly" do
      expect(@resource['notes'].select { |n| n['type'] == 'odd' }.map { |n| n['persistent_id'] }.sort).to eq(['ref45', 'ref44', 'ref15'].sort)
    end
  ...
  describe 'DAO and DAOGROUPS' do
    ...
    it 'should honor xlink:show and xlink:actuate from arc elements' do
      expect(@file_versions[0..2].map { |fv| fv['xlink_actuate_attribute'] }).to eq(['onLoad', 'onRequest', 'onLoad'])
      expect(@file_versions[0..2].map { |fv| fv['xlink_show_attribute'] }).to eq(['new', 'embed', 'new'])
    end
