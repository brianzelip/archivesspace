...
class RequestItem < Struct.new(:user_name, :user_email, :date, :note,
  ...
  def self.allow_for_type(repository_code, record_type)
  ...
  def self.allow_nontops(repo_code)
    allow = nil
    rep_allow = nil
    begin
      ...
      raise e unless e.message.start_with?('No value set for config parameter')
    end
  ...
  def to_text_array(skip_empty = false)
    arr = []
    [:user_name, :user_email, :date, :note, :title, :identifier, :cite, :request_uri, :resource_name, :resource_id, :repo_name, :hierarchy, :restrict].each do |sym|
      arr.push("#{sym}: #{self[sym]}") unless skip_empty && self[sym].blank?
    end
    arr.push("machine: #{self[:machine].blank? ? '' : self[:machine].join(', ')}")
    if !self[:container].blank? && !self[:container].empty?
      self[:container].each_with_index do |_v, i|
        #        arr.push("#{:container.to_s}: #{v}")
        [:container, :top_container_url, :barcode, :location_title, :location_url].each do |sym|
          arr.push("#{sym}: #{defined?(self[sym][i]) ? self[sym][i] : ''}")
        end
      end
    elsif !skip_empty
      [:container, :top_container_url, :barcode, :location_title, :location_url].each { |sym| arr.push("#{sym}:") }
    end
