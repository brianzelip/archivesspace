...
def create_language_record_from_language_id(record_type, dataset, _language_id)
  dataset.each do |row|
...
def migrate_langmaterial_notes
  ...
  self[:note].each do |note_id|
    next unless note_id[:notes].lit.include?('langmaterial')
    ...
    [:resource_id, :archival_object_id, :digital_object_id, :digital_object_component_id].each do |obj|
      ...
      if note.lit.include?('<language')
        ...
        split = split[1].tr('"]}', '')
        parsed = begin
                       Nokogiri::XML::DocumentFragment.parse(split.to_s)
                 rescue StandardError
                   nil
                     end
        languages = begin
                          parsed.xpath('.//language')
                    rescue StandardError
                      nil
                        end
        ...
        content = note.gsub(%r{(<language langcode=\\"[a-z]+\\" scriptcode=\\"[A-z]+\\">(.[^<>]*)</language>)|(<language langcode=\\"[a-z]+\\">(.[^<>]*)</language>)|(<language langcode=\\"[a-z]+\\"/>)}, '\\2\\4')
