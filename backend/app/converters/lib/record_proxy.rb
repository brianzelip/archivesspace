module ASpaceImport
  class RecordProxyMgr
    ...
    def get_proxy_for(proxy_key, record_type = nil)
      @proxies[proxy_key] = RecordProxy.new(proxy_key, record_type) unless @proxies.has_key?(proxy_key)
    ...
    def undischarged
      undis = []
      @proxies.each do |_type, proxy|
        undis << proxy unless proxy.discharged
      end
      undis
    end
  end
  ...
  class RecordProxy
    ...
    def initialize(key, record_type = nil)
      ...
      @discharged = false
      @data = {}
      @record_type = record_type if record_type
    end
    ...
    def on_discharge(receiver, method, *args)
      @jobs << proc { |obj| receiver.send(method, *args, obj) }
    end
