...
class DB
  ...
  class DBPool
    ...
    def connect
      unless @pool
        ...
        check_supported(AppConfig[:db_url]) unless AppConfig[:allow_unsupported_database]
        ...
        begin
          ...
          ensure_tables_are_utf8(pool) if !@opts[:skip_utf8_check] && pool.database_type == :mysql && AppConfig[:allow_non_utf8_mysql_database] != 'true'
    ...
    def session_storage
      Thread.current[:db_session_storage] || raise('Not inside transaction!')
    end
    ...
    def open(transaction = true, opts = {})
      ...
      begin
        ...
        Thread.current[:db_session_storage] = nil if Thread.current[:nesting_level] <= 0
      end
    end
    ...
    def system_metadata
      RbConfig.const_get('CONFIG').select { |key|
        ['host_os', 'host_cpu',
    ...
    def is_integrity_violation(exception)
      (exception.wrapped_exception.cause || exception.wrapped_exception).getSQLState =~ /^23/
    end
    ...
    def is_retriable_exception(exception, opts = {})
      ...
      (exception.instance_of?(RetryTransaction) ||
       ...
       (exception.wrapped_exception && (exception.wrapped_exception.cause || exception.wrapped_exception).getSQLState =~ /^(40|41)/))
    end
    ...
    def check_supported(url)
      if SUPPORTED_DATABASES.none? { |db| url =~ db[:pattern] }
    ...
    def expire_backups
      ...
      Dir.foreach(backups_dir) do |filename|
        backups << File.join(backups_dir, filename) if filename =~ /^demo_db_backup_[0-9]+_[0-9]+$/
      end
    ...
    def increase_lock_version_or_fail(obj)
      ...
      raise Sequel::Plugins::OptimisticLocking::Error, "Couldn't create version of: #{obj}" if updated_rows != 1
    end
  ...
  DBPool.instance_methods(false).each do |method|
    next if singleton_methods(false).include?(method)
