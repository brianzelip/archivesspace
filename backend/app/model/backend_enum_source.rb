...
class BackendEnumSource
  def self.valid?(enum_name, value)
    if RequestContext.get(:create_enums)
      ...
      unless Array(created[enum_name]).include?(value)
        # The customer is always right!
        DB.open(true) do |db|
          enum_id = db[:enumeration].filter(name: enum_name).select(:id).first[:id]

          raise "Couldn't find enum: #{enum_name}" unless enum_id
      ...
      @@enum_value_cache[enum_name] = {
        ...
        entry: DB.open(true) do |db|
          ...
          db[:enumeration].join(:enumeration_value, enumeration_id: :id)
            ...
            editable = ((row[:editable] === 1) || (row[:editable] == true))
          end
  ...
  def self.values_for(enum_name)
    cache_entry_for(enum_name)[:values]
  end

  def self.editable?(enum_name)
    ((cache_entry_for(enum_name)[:editable] === 1) || (cache_entry_for(enum_name)[:editable] == true))
  end

  def self.id_for_value(enum_name, value)
    result = cache_entry_for(enum_name)[:value_to_id_map][value]

    result ||= cache_entry_for(enum_name, true)[:value_to_id_map][value]
  ...
  def self.value_for_id(enum_name, id)
    ...
    result = cache_entry_for(enum_name)[:id_to_value_map][id]

    cache_entry_for(enum_name, true)[:id_to_value_map][id] unless result
