class ArchivesSpaceService < Sinatra::Base
  ...
  do
    ...
    raise BadParamsException.new(merge_request: ['Agent merge request can only merge agent records']) if (victims.map { |r| r[:type] } + [target[:type]]).any? { |type| !AgentManager.known_agent_type?(type) }
  ...
  do
    ...
    raise BadParamsException.new(merge_request_detail: ['Agent merge request can only merge agent records']) if (victims.map { |r| r[:type] } + [target[:type]]).any? { |type| !AgentManager.known_agent_type?(type) }
    ...
    if params[:dry_run]
      ...
      target.update_from_json(new_target) if selections != {}
      json_response(status: 'OK')
    end
  ...
  def check_repository(target, victims, repo_id)
    ...
    raise BadParamsException.new(merge_request: ['All records to merge must be in the repository specified']) if ([target] + victims).any? { |r| r[:repository] != repo_uri }
  end

  def ensure_type(target, victims, type)
    raise BadParamsException.new(merge_request: ["This merge request can only merge #{type} records"]) if (victims.map { |r| r[:type] } + [target[:type]]).any? { |t| t != type }
  end
  ...
  def merge_details(target, victim, selections, dry_run)
    ...
    selections.each_key do |key|
      ...
      path.each do |part|
        if part.length === 1
          part = part.to_i
        elsif (part.length === 2) && part.start_with?('1')
          part = part.to_i
        end
        path_fix.push(part)
      end
      ...
      if path_fix[0] != 'related_agents' && path_fix[0] != 'external_documents' && path_fix[0] != 'notes'
        ...
        when 3
          begin
            target[path_fix[0]].push(victim[path_fix[0]][path_fix[1]]) if target[path_fix[0]].length <= path_fix[1]
            target[path_fix[0]][path_fix[1]][path_fix[2]] = victim[path_fix[0]][path_fix[1]][path_fix[2]]
          rescue StandardError
            target[path_fix[0]].push(victim[path_fix[0]][path_fix[1]]) if target[path_fix[0]] === []
          end
        when 4
  ...
  def preview_sort_name(target)
    ...
    when 'name_corporate_entity'
      ...
      result << " (#{grouped.join(' : ')})" unless grouped.empty?
    when 'name_family'
