class Repository < Sequel::Model(:repository)
  ...
  def after_create
    if repo_code == Repository.GLOBAL
  ...
  def delete
    ...
    [Classification, Event, Resource, DigitalObject, Accession].each do |klass|
      klass.send(:filter, repo_id: id).destroy
    end
  ...
  def self.sequel_to_jsonmodel(objs, opts = {})
    ...
    jsons.zip(objs).each do |json, obj|
      ...
      next unless (agent_id = obj.agent_representation_id)
  ...
  def update_from_json(json, opts = {}, apply_nested_records = true)
    reindex_required = publish != (json['publish'] ? 1 : 0)
    classification_reindex_required = name != json['name']
  ...
  def reindex_repository_records
    ASModel.all_models.each do |model|
      model.update_mtime_for_repo_id(id) if model.model_scope(true) == :repository && model.publishable?
    end
  end

  def reindex_classification_records
    ClassificationTerm.update_mtime_for_repo_id(id)
    Classification.update_mtime_for_repo_id(id)
  end
end
