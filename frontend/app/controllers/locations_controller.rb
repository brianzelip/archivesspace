class LocationsController < ApplicationController
  ...
  LOCATION_STICKY_PARAMS = ['building', 'floor', 'room', 'area'].freeze
  ...
  def new
    location_params = params.each_with_object({}) { |(k, v), c| c[k] = v if LOCATION_STICKY_PARAMS.include?(k); }
  ...
  def batch_create
    if params[:location_batch][:record_uris] && !params[:location_batch][:record_uris].empty?
      ...
      uri += '?dry_run=true' if params['dry_run']
      response = JSONModel::HTTP.post_json(URI(uri), batch.to_json)

      batch_response = ASUtils.json_parse(response.body)
    end

    if batch_response.is_a?(Hash) && batch_response.has_key?('error')
      if params['dry_run']
    ...
    if params['dry_run']
      ...
      flash[:success] = if @location_batch.jsonmodel_type == 'location_batch_update'
                          I18n.t('location_batch._frontend.messages.updated', number_created: batch_response.length)
                        else
                          I18n.t('location_batch._frontend.messages.created', number_created: batch_response.length)
                        end
