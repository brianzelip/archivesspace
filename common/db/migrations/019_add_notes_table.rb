...
Sequel.migration do
  up do
    ...
    TABLES_WITH_NOTES = [:resource, :archival_object, :digital_object, :digital_object_component,
                         :agent_person, :agent_corporate_entity, :agent_family, :agent_software].freeze

    TABLES_WITH_NOTES.each do |table|
      warn("Migrating notes for records of type #{table}")
      migrated_count = 0
      ...
      self[table].filter(Sequel.~(notes: nil)).each_by_page do |row|
        ASUtils.json_parse(row[:notes]).each do |note|
          ...
          warn("Migrated #{migrated_count} #{table} records") if (migrated_count % 1000) == 0
          migrated_count += 1
        end
      end
    end
