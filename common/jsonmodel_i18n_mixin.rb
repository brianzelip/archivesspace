    ...
    unless already_translated
      [:errors, :warnings].each do |level|
        ...
        exceptions[level].clone.each do |path, msgs|
          exceptions[level][path] = if path == 'conflicting_record'
                                      msgs
                                    else
                                      msgs.map { |m| translate_exception_message(m) }
                                    end
        end
      end
  ...
  def translate_exception_message(msg)
    msg_data = case msg
               ...
               when /^Username '(.+)' is already in use/
                 [:username_already_in_use, { username: Regexp.last_match(1) }]
               when /^Did not match regular expression: (.+)/
                 [:did_not_match_regular_expression, { regexp: Regexp.last_match(1) }]
               when /^Must be at least ([0-9]+) characters/
                 [:too_few_characters, { min_length: Regexp.last_match(1) }]
               when /^Must be ([0-9]+) characters or fewer/
                 [:too_many_characters, { max_length: Regexp.last_match(1) }]
               when /^At least ([0-9]+) item\(s\) is required/
                 [:too_few_items, { min_items: Regexp.last_match(1) }]
               when /^Invalid value '(.*?)'.  Must be one of: (.*)/
                 [:invalid_value, { value: Regexp.last_match(1), valid_set: Regexp.last_match(2) }]
               when /^Must be a (.*?) \(you provided a (.*)\)/
                 [:wrong_type, { desired_type: Regexp.last_match(1), actual_type: Regexp.last_match(2) }]
               when /^Must be one of: (.*?) \(you provided a (.*)\)/
                 [:must_be_one_of, { allowed_types: Regexp.last_match(1), actual_type: Regexp.last_match(2) }]
               when /Username '(.*)' is already in use/
                 [:username_already_in_use, { username: Regexp.last_match(1) }]
               else
