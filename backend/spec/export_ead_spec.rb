...
describe 'EAD export mappings' do
  ...
  shared_examples 'archival object desc mappings' do
    ...
    it 'maps {archival_object}.extent to {desc_path}/did/physdesc' do
      ...
      object.extents.each do |ext|
        ...
        mt(ext['container_summary'], "#{desc_path}/did/physdesc[#{count}]/extent[@altrender='carrier']") if ext['container_summary']
        mt(ext['dimensions'], "#{desc_path}/did/physdesc[#{count}]/dimensions") if ext['dimensions']
        mt(ext['physical_details'], "#{desc_path}/did/physdesc[#{count}]/physfacet") if ext['physical_details']
        count += 1
      end
    end
    ...
    describe 'How {archival_object}.notes data are mapped >> ' do
      ...
      it "maps notes of type 'langmaterial' to did/langmaterial" do
        notes.select { |n| n['type'] == 'langmaterial' }.each_with_index do |note, _i|
          content = note_content(note)
          path = "#{desc_path}/did/langmaterial[text()='#{content}']"
          if note['persistent_id']
            mt('aspace_' + note['persistent_id'], path, 'id')
  ...
  describe '/eadheader mappings' do
    ...
    it 'maps resource.ead_location to eadid/@url' do
      mt(@resource.ead_location, 'eadheader/eadid', 'url') unless AppConfig[:arks_enabled]
    end
  ...
  describe 'ARK URLs' do
    it 'maps ARK URL to eadid/@url if ARK URLs are enabled' do
      expect(@doc_with_ark.to_s).to match(%r{<eadid.*url=\"http.*/ark:})
    end
    it 'does not map ARK URL to eadid/@url if ARK URLs are disabled' do
      expect(@doc_ark_disabled.to_s).to_not match(%r{<eadid.*url=\"http.*/ark:})
    end
  ...
  describe 'How digital_objects are mapped to <dao> nodes >> ' do
    ...
    it 'displays file_uri if file version is published, digital_object_id otherwise' do
      ...
      digital_objects.each do |d|
        ...
        if visible_file_versions.empty?
