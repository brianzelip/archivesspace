class Sequence
  ...
  def self.get(sequence)
    DB.open(true) do |db|
      ...
      unless Thread.current[:initialised_sequences][sequence]
      ...
      1000.times do
        ...
        if updated_count == 0
          ...
          raise SequenceError, "Unrecognised response from SQL update when generating next element in sequence '#{sequence}': #{updated_count}"
        end
      end

      raise SequenceError, "Gave up trying to generate a sequence number for: '#{sequence}'"
    end
  end
end
