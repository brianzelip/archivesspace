...
module JSONModel
  ...
  def self.JSONModel(source)
    load_schema(source.to_s) unless @@models.has_key?(source.to_s)

    @@models[source.to_s] || raise("JSONModel not found for #{source}")
  end
  ...
  def self.repository_for(reference)
    Regexp.last_match(1) if reference =~ %r{^(/repositories/[0-9]+)/}
  end
  ...
  # cleverer...
  #
  REFERENCE_KEY_REGEX = %r{(/[0-9]+)}.freeze
  ...
  def self.schema_src(schema_name)
    raise "Invalid schema name: #{schema_name}" if schema_name.to_s !~ /\A[0-9A-Za-z_-]+\z/
    ...
    [*ASUtils.find_local_directories('schemas'),
     File.join(File.dirname(__FILE__), 'schemas')].each do |dir|
      ...
      return File.open(schema).read if File.exist?(schema)
    end
  ...
  def self.load_schema(schema_name)
    unless @@models[schema_name]
      ...
      allow_unmapped_enum_value(entry[:schema]['properties']) if @@init_args[:allow_other_unmapped]
      ...
      create_model_for(schema_name, entry[:schema])
    end
  end
  ...
  def self.check_valid_refs(properties)
    if properties.is_a?(Hash)
      properties.each do |key, value|
        if key == 'ref'
          raise "ref value should be an object.  Got type: #{value.class}" unless value.is_a?(Hash)
        else
  ...
  def self.init(opts = {})
    ...
    return true if @@init_args

    @@strict_mode = true if opts.has_key?(:strict_mode)

    @@init_args = opts

    unless opts.has_key?(:enum_source)
      if opts[:client_mode]
  ...
  def self.parse_jsonmodel_ref(ref)
    [Regexp.last_match(1).intern, Regexp.last_match(2)] if ref.is_a?(String) && ref =~ /JSONModel\(:([a-zA-Z_\-]+)\) (.*)/
  end
