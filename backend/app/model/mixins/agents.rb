...
module Agents
  ...
  module ClassMethods
    ...
    def initialize_enum(clz)
      (role_enum = @agent_role_enum) || raise("Missing agent_role_enum in #{self}")
      relator_enum = @agent_relator_enum

      raise "Missing agent_relator_enum in #{self}" if columns.include?(:relator_id) && relator_enum.nil?
    ...
    def initialize_terms(clz)
      clz.instance_eval do
        ...
        define_method(:values) do
          ...
          result['terms'] = Array(term).map { |term| Term.to_jsonmodel(term) }
