...
describe 'EAD3 export mappings' do
  ...
  def load_export_fixtures
    ...
    resource = create(:json_resource, linked_agents: build_linked_agents(@agents),
                                      ...
                                      subjects: @subjects.map { |ref, _s| { ref: ref } },
                                      instances: instances,
      ...
      a = create(:json_archival_object_normal, resource: { ref: @resource.uri },
                                               ...
                                               instances: [build(:json_instance_digital),
                                                           build(:json_instance,
                                                                 sub_container: build(:json_sub_container,
                                                                                      top_container: { ref: @top_container.uri }))],
                                               subjects: @subjects.map { |ref, _s| { ref: ref } }.shuffle,
                                               publish: true)
  ...
  def mt(*args)
    ...
    doc_to_test = args[1]&.match(%r{/xmlns:[\w]+}) ? @doc : @doc_nsless
  ...
  def build_linked_agents(agents)
    agents = agents.map { |ref, _a|
      {
        ref: ref,
        role: (ref[-1].to_i.even? ? 'creator' : 'subject'),
  ...
  describe '/control mappings' do
    ...
    it 'maps resource.ead_location to recordid/@instanceurl' do
      mt(@resource.ead_location, 'control/recordid', 'instanceurl') unless AppConfig[:arks_enabled]
    end
    ...
    describe 'repository.agent.agent_contacts[0] to filedesc/publicationstmt/address/ mappings' do
      ...
      let(:offset_2) { !['city', 'region', 'post_code'].map { |k| contact[k] }.compact.empty? ? 1 : 0 }
      ...
      it 'maps city, region, post_code to addressline' do
        line = ['city', 'region'].map { |k| contact[k] }.compact.join(', ')
        line += " #{contact['post_code']}"
        line.strip!

        mt(line, "#{path}addressline[#{offset_1}]") unless line.empty?
      end
  ...
  shared_examples 'archival object desc mappings' do
    ...
    it 'maps {archival_object}.(id_[0-3]|component_id) to {desc_path}/did/unitid' do
      mt(unitid_src, "#{desc_path}/did/unitid") if !unitid_src.nil? && !unitid_src.empty?
    end
    ...
    it 'maps {archival_object}.lang_materials to {desc_path}/did/langmaterial' do
      ...
      language_notes.select { |n| n['type'] == 'langmaterial' }.each_with_index do |note, _i|
    ...
    it 'maps {archival_object}.extent to {desc_path}/did/physdesc' do
      ...
      object.extents.each do |ext|
        mt(ext['number'], "#{desc_path}/did/physdescstructured[#{count}]/quantity") if ext['number']
        ...
        mt(ext['dimensions'], "#{desc_path}/did/physdescstructured[#{count}]/dimensions") if ext['dimensions']

        mt(ext['physical_details'], "#{desc_path}/did/physdescstructured[#{count}]/physfacet") if ext['physical_details']

        mt(ext['container_summary'], "#{desc_path}/did/physdesc[#{count}]") if ext['container_summary']
    ...
    it 'maps structured {archival_object}.date to {desc_path}/did/unitdatestructured and applies correct attributes' do
      ...
      object.dates.each do |date|
        if date['begin'] && date['date_type'] == 'single'
          ...
          value = date['expression'] || date['begin']
          ...
          mt(date['certainty'], unitdatestructured_path, 'certainty') if date['certainty']

          mt(date['era'], unitdatestructured_path, 'era') if date['era']

          mt(date['calendar'], unitdatestructured_path, 'calendar') if date['calendar']
    ...
    it 'maps date expression to {archival_object}.date to {desc_path}/did/unitdate when present along with both start and end dates' do
      count = 1
      object.dates.each do |date|
        next unless date['begin'] && date['end'] && date['expression']
    ...
    describe 'How {archival_object}.notes data are mapped >> ' do
      ...
      xit "maps notes of type 'dimensions' to did/physdesc" do
        notes.select { |n| n['type'] == 'dimensions' }.each_with_index do |note, _i|
      ...
      xit "maps notes of type 'physfacet' to did/physdesc" do
        notes.select { |n| n['type'] == 'physfacet' }.each_with_index do |note, _i|
          content = note_content(note)
          path = "#{desc_path}/did/physdesc[text()='#{content}']"
    ...
    describe 'archdesc or component notes section: ' do
      let(:archdesc_note_types) {
        ['accruals', 'appraisal', 'arrangement', 'bioghist', 'accessrestrict', 'userestrict', 'custodhist', 'altformavail', 'originalsloc', 'fileplan', 'odd', 'acqinfo', 'otherfindaid', 'phystech', 'prefercite', 'processinfo', 'relatedmaterial', 'scopecontent', 'separatedmaterial']
      }

      it 'maps note content to {desc_path}/NOTE_TAG' do
        object.notes.select { |n| archdesc_note_types.include?(n['type']) }.each do |note|
          head_text = note['label'] || translate('enumerations._note_types', note['type'])
    ...
    describe 'How mixed content notes are mapped >> ' do
      ...
      it 'ensures subnotes[] order is respected, even if subnotes are of mixed types' do
        ...
        @mixed_subnotes_tracer['subnotes'].each do |note|
          mt(/#{note["jsonmodel_type"]}/, "#{path}/*[text() != ''][#{i}]")
          i += 1
        end
      end
    ...
    describe 'How the <controlled> access section gets built >> ' do
      ...
      it "maps linked agents with role 'subject' or 'source' to {desc_path}/controlaccess/NODE" do
        object.linked_agents.each do |link|
          ...
          next unless ['source', 'subject'].include?(link_role)

          relator = link[:relator] || link['relator']
          role = relator || (link_role == 'source' ? 'fmo' : nil)
  ...
  describe 'How linked agents are mapped to the ead/archdesc/did section >> ' do
    it "maps linked agents with role of 'source' or 'creator' to archdesc/did/origination/(pers|fam|corp)name" do
      @resource.linked_agents.each do |link|
        role = link[:role]
        next unless ['source', 'creator'].include?(role)
  ...
  describe 'How digital_objects are mapped to <dao> nodes >> ' do
    ...
    it 'maps each resource.instances[].instance.digital_object to archdesc/dao' do
      digital_objects.each do |obj|
        next if obj['file_versions'].empty?
        ...
        obj['file_versions'].each do |fv|
          ...
          mt(fv['use_statement'], path, 'localtype') if fv['use_statement']
