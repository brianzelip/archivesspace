...
describe 'OAI handler' do
  ...
  XML_HEADER = '<!-- THIS FILE IS AUTOMATICALLY GENERATED - DO NOT EDIT.  To update it, just delete it and it will be regenerated from your test data. -->'.freeze
  ...
  def check_oai_request_against_fixture(fixture_name, params)
    ...
    if !File.exist?(fixture_file)
      warn("NOTE: Updating fixture #{fixture_file}")
  ...
  describe 'OAI protocol and mapping support' do
    ...
    RESOURCE_BASED_FORMATS = ['oai_ead'].freeze
    RESOURCE_AND_COMPONENT_BASED_FORMATS = ['oai_dc', 'oai_dcterms', 'oai_mods', 'oai_marc'].freeze
    ...
    it 'includes sets defined in OAIConfig table in ListSets request' do
      ...
      expect(result).to match(%r{<setSpec>sponsor_set_name</setSpec><setName>sponsor_set_name</setName>})
      expect(result).to match(%r{<setSpec>repo_set_name</setSpec><setName>repo_set_name</setName>})
      expect(result).to match(%r{<oai_dc:description.*>bimbaz</oai_dc:description>})
      expect(result).to match(%r{<oai_dc:description.*>foobar</oai_dc:description>})
    end
  ...
  describe 'ListRecords' do
    ...
    it 'lists deletes' do
      ...
      loop do
        ...
        opts[:resumption_token] = token if token
    ...
    it "doesn't reveal published or suppressed records" do
      ...
      loop do
        ...
        records.each do |record|
          raise "URI #{record.jsonmodel_record.uri} is unpublished/suppressed and should not be shown in OAI results" if record.is_a?(ArchivesSpaceOAIRecord) && prohibited_uris.include?(record.jsonmodel_record.uri)
        end
  ...
  describe 'OAI mappers output' do
    describe 'DC output' do
      it 'should map Conditions Governing Access and Conditions Governing Use to <dc:rights>' do
        ...
        response = get uri
        expect(response.body).to match(%r{<dc:rights.*>conditions governing access note</dc:rights>})
        expect(response.body).to match(%r{<dc:rights.*>conditions governing use note</dc:rights>})

        expect(response.body).not_to match(%r{<dc:relation>conditions governing access note</dc:relation>})
        expect(response.body).not_to match(%r{<dc:relation>conditions governing use note</dc:relation>})
      end
      ...
      it 'should map Extents to dc:format, not dc:extent' do
        ...
        expect(response.body).to match(%r{<dc:format.*>10 Volumes; Container summary</dc:format>})
        expect(response.body).to match(%r{<dc:format.*>physical description note</dc:format>})
        expect(response.body).to match(%r{<dc:format.*>dimensions note</dc:format>})

        expect(response.body).not_to match(%r{<dc:extent>10 Volumes; Container summary</dc:extent>})
        expect(response.body).not_to match(%r{<dc:extent>physical description note</dc:extent>})
        expect(response.body).not_to match(%r{<dc:extent>dimensions note</dc:extent>})
      end
      it 'should output ARK name in identifer tag' do
        ...
        expect(response.body).to match(%r{<dc:identifier.*>#{ark_url}</dc:identifier>})
      end
      it 'should not output ARK name in identifer tag if ARK output is disabled' do
        ...
        resource_id = @test_resource_record.split('/')[4]
        ark_url = ArkName.get_ark_url(resource_id.to_i, :resource)
        expect(response.body).to_not match(%r{<dc:identifer.*>#{ark_url}</dc:identifer>})
    ...
    describe 'DCTerms output' do
      xit 'should output ARK name in identifer tag' do
        ...
        resource_id = @test_resource_record.split('/')[4]
        ark_url = ArkName.get_ark_url(resource_id.to_i, :resource)
        expect(response.body).to match(%r{<dcterms:identifer.*>#{ark_url}</dcterms:identifer>})
      end
      it 'should not output ARK name in identifer tag if ARK output is disabled' do
        AppConfig[:arks_enabled] = false
        uri = "/oai?verb=GetRecord&identifier=oai:archivesspace/#{@test_resource_record}&metadataPrefix=oai_dcterms"
        ...
        resource_id = @test_resource_record.split('/')[4]
        ark_url = ArkName.get_ark_url(resource_id.to_i, :resource)
        expect(response.body).to_not match(%r{<dcterms:identifer.*>#{ark_url}</dcterms:identifer>})
        AppConfig[:arks_enabled] = true
      end
    end
    describe 'MODS output' do
      it 'should output ARK name in identifer tag' do
        ...
        resource_id = @test_resource_record.split('/')[4]
        ark_url = ArkName.get_ark_url(resource_id.to_i, :resource)
        expect(response.body).to match(%r{<identifier.*>#{ark_url}</identifier>})
      end
    end
  end
  ...
  describe 'repository with sets disabled' do
    ...
    it 'returns an object if set included in OAI in repo' do
      ...
      expect(!doc.xpath("//xmlns:header[not(@status='deleted')]").empty?).to be true
    end
    ...
    it 'does not list a set in ListSets if that set is not enabled for at least one repository' do
      ...
      expect(doc.to_s).to match(%r{<set><setSpec>fonds</setSpec>})

      expect(doc.to_s).to_not match(%r{<set><setSpec>class</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>collection</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>file</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>item</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>otherlevel</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>recordgrp</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>series</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>subfonds</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>subgrp</setSpec>})
      expect(doc.to_s).to_not match(%r{<set><setSpec>subseries</setSpec>})
    end
  end
  ...
  describe 'repository without sets disabled' do
    it 'does list a set in ListSets if that set is enabled for at least one repository' do
      ...
      expect(doc.to_s).to match(%r{<set><setSpec>fonds</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>class</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>collection</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>file</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>item</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>otherlevel</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>recordgrp</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>series</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>subfonds</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>subgrp</setSpec>})
      expect(doc.to_s).to match(%r{<set><setSpec>subseries</setSpec>})
    end
  end
end
