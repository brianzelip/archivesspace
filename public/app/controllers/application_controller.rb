class ApplicationController < ActionController::Base
  ...
  unless ASUtils.find_local_directories.blank?
    ASUtils.find_local_directories.map { |local_dir| File.join(local_dir, 'public', 'views') }.select { |dir| Dir.exist?(dir) }.each do |template_override_directory|
  ...
  def render_no_results_found(exception)
    ...
    if controller_name == 'repositories'
      redirect_to('/')
    else
  ...
  def process_slug_or_id(params)
    ...
    # if we have 'slug' strings that are integers, treat them like IDs.
      ...
      params[:rid] = params[:repo_slug] if params[:repo_slug]&.match(/^(\d)+$/)
  ...
  def resolve_ids_with_slugs(params)
    ...
    params_from_response(params, json_response)
  end
