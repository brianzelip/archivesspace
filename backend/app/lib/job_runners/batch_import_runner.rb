  ...
  def run
            ...
            begin
              RequestContext.open(create_enums: true,
                ...
                File.open(converter.get_output_path, 'r') do |fh|
                  ...
                  created_uris.concat(batch.created_records.values) if batch.created_records
    ...
    if last_error
      ...
      if last_error.respond_to?(:errors)
        ...
        last_error.errors.each_pair { |k, v| ticker.log("\t#{k} : #{v.join(' -- ')}") }
        ...
        ticker.log("\n\n For #{last_error.invalid_object.class}: \n #{last_error.invalid_object.inspect}") if last_error.respond_to?(:invalid_object) && last_error.invalid_object
