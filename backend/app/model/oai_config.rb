class OAIConfig < Sequel::Model(:oai_config)
  ...
  def validate_single_record
    ...
    unless record_count == 0
      ...
      errors.add(:base, 'Cannot have more than one record in oai_config table.') if id.nil? || first_record.id != id
    end
  end

  def validate_oai_admin_email_is_email
    errors.add(:oai_admin_email, 'must be a valid email address') unless oai_admin_email =~ /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i
  end

  def validate_repo_set_fields
    errors.add(:repo_set_description, 'repo_set_desc_required') if repo_set_codes && repo_set_codes != '[]' && !repo_set_description

    errors.add(:repo_set_name, 'repo_set_name_required') if repo_set_codes && repo_set_codes != '[]' && !repo_set_name
  end

  def validate_sponsor_set_fields
    errors.add(:sponsor_set_description, 'sponsor_set_desc_required') if sponsor_set_names && sponsor_set_names != '[]' && !sponsor_set_description

    errors.add(:sponsor_set_name, 'sponsor_set_name_required') if sponsor_set_names && sponsor_set_names != '[]' && !sponsor_set_name
  end
end
