class METSModel < ASpaceExport::ExportModel
  ...
  def dmd_wrap(mdtype, callback, data)
    wrapped_dmd << { 'type' => mdtype, 'callback' => callback, 'data' => data }
  end
  ...
  def extract_file_versions(children)
    ...
    children.each do |child|
      file_versions += child['file_versions'] if child['file_versions']
      file_versions += extract_file_versions(child['children']) if child['children']
    end
