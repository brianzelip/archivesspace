class EADModel < ASpaceExport::ExportModel
  ...
  RESOLVE = ['subjects', 'linked_agents', 'digital_object', 'top_container', 'top_container::container_profile'].freeze
  ...
  @data_src = Class.new do
    ...
    def method_missing(meth)
      if @json.respond_to?(meth)
        @json.send(meth)
      elsif @json.is_a?(Hash) && @json.has_key?("#{meth}")
        @json["#{meth}"]
      end
    end
  end
  ...
  class IndexedArchivalObjectPrefetcher
    ...
    def fetch(ao_ids, resolve)
      ...
      indexed_records.each do |indexed|
        ...
        next unless desired_version == indexed_version
  ...
  @ao = Class.new do
    ...
    def method_missing(meth, *args)
      @json.send(meth, *args) if @json.respond_to?(meth)
    end

    def creators_and_sources
      linked_agents.select { |link| ['creator', 'source'].include?(link['role']) }
    end

    def instances_with_sub_containers
      instances.select { |inst| inst['sub_container'] }.compact
    end

    def instances_with_digital_objects
      instances.select { |inst| inst['digital_object'] }.compact
    end
  end
  ...
  def addresslines
    ...
    ['telephone', 'email'].each do |property|
  ...
  def descrules
    return nil unless @descrules || finding_aid_description_rules

    @descrules ||= I18n.t("enumerations.resource_finding_aid_description_rules.#{finding_aid_description_rules}", default: finding_aid_description_rules)
    @descrules
  end
  ...
  def digital_objects
    if @include_daos
      instances.select { |inst| inst['digital_object'] }.compact.map { |inst| inst['digital_object']['_resolved'] }.compact
    else
      []
    end
  end
