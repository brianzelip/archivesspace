...
class EADSerializer < ASpaceExport::Serializer
  ...
  def handle_linebreaks(content)
    ...
    return content if content.strip =~ %r{^<p(\s|/|>)} || content.strip.empty?

    original_content = content
    blocks = content.split("\n\n").reject { |b| b.strip.empty? }
    content = if blocks.length > 1
                blocks.inject('') do |c, n|
                  c << "<p>#{escape_content(n.chomp)}</p>"
                end
              else
                "<p>#{escape_content(content.strip)}</p>"
              end
  ...
  def add_xlink_prefix(content)
    ['actuate', 'arcrole', 'entityref', 'from', 'href', 'id', 'linktype', 'parent', 'role', 'show', 'target', 'title', 'to', 'xpointer'].each do |xa|
  ...
  def sanitize_mixed_content(content, context, fragments, allow_p = false)
    ...
    xlink_eles = ['arc', 'archref', 'bibref', 'extptr', 'extptrloc', 'extref', 'extrefloc', 'linkgrp', 'ptr', 'ptrloc', 'ref', 'refloc', 'resource', 'title']
          ...
          if ark_url
            ...
            xml.unitid {
              xml.extref ({ 'xlink:href' => ark_url,
                            ...
                            'xlink:show' => 'new',
                            'xlink:type' => 'simple' }) { xml.text 'Archival Resource Key' }
            }
          end
        end
  ...
  def serialize_subnotes(subnotes, xml, fragments, include_p = true)
    subnotes.each do |sn|
      ...
      when 'note_orderedlist'
        atts = { type: 'ordered', numeration: sn['enumeration'] }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }.merge(audatt)
        xml.list(atts) {
          xml.head { sanitize_mixed_content(title, xml, fragments) } if title
  ...
  def serialize_did_notes(data, xml, fragments)
    data.notes.each do |note|
      ...
      att = { id: prefix_id(note['persistent_id']) }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }
      att ||= {}
  ...
  def serialize_note_content(note, xml, fragments)
    ...
    atts = { id: prefix_id(note['persistent_id']) }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }.merge(audatt)
  ...
  def serialize_eadheader(data, xml, fragments)
    ...
    eadheader_atts = { findaidstatus: data.finding_aid_status,
                       ...
                       langencoding: 'iso639-2b' }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }

    xml.eadheader(eadheader_atts) {
      eadid_atts = { countrycode: data.repo.country,
                     url: eadid_url,
                     mainagencycode: data.mainagencycode }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }
      ...
      unless export_rs.empty?
        xml.revisiondesc {
          export_rs.each do |rs|
            if rs['description']&.strip&.start_with?('<')
