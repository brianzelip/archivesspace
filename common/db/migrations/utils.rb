...
module EachByPage
  def each_by_page(page_size = 1000)
    extension(:pagination).each_page(page_size) do |page_ds|
      page_ds.each do |row|
        yield(row)
      end
    end
...
def create_enum(name, values, default = nil, editable = false, opts = {})
  ...
  include_position = schema(:enumeration_value).flatten.include?(:position)
  ...
  self[:enumeration].where(id: id).update(default_value: id_of_default) unless id_of_default.nil?
end
