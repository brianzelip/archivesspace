...
class UserDefinedFieldMigrator
  ...
  def apply_update(top_container, container_data)
    ...
    if dry_run?
      ...
      Log.info("DRY RUN: Linked records needed user-defined fields: #{find_records_needing_user_defined_fields(top_container).inspect}") if container_data.voyager_bib_id
    else
  ...
  def migrate
    ...
    raise "No repository found for repo_code '#{AppConfig[:at_target_aspace_repo]}'" unless repo
      ...
      Sequel.connect(AppConfig[:at_db_url]) do |db|
        ...
        RequestContext.open(repo_id: repo.id,
          ...
          TopContainer.filter(repo_id: repo.id).each do |top_container|
            ...
            Log.info("Processing top container #{count} of #{total_count}") if (count % 100) == 0
            ...
            DB.open do |db|
              ...
              if got_a_match
                apply_update(top_container, container_data) if container_data
              else
  ...
  def find_records_needing_user_defined_fields(top_container)
    ...
    ASModel.all_models.each do |model|
      next unless model.included_modules.include?(Instances)
...
class InstanceKey < Struct.new(:resource_id, :resource_component_id, :barcode, :indicator)
  def self.from_row(row)
    ...
    if barcode
      ...
      indicator = format('%g', row[:container1NumericIndicator])
    end
...
class InstanceData < Struct.new(:voyager_info, :box_type, :restricted, :exported_to_voyager)
  UNKNOWN_DIMENSION = '99999999'.freeze
  ...
  def voyager_bib_id
    self[:voyager_info].split('_')[0] if voyager_info && !voyager_info.empty?
  end

  def voyager_holding_id
    self[:voyager_info].split('_')[1] if voyager_info && !voyager_info.empty?
  end

  def container_profile
    return nil if !box_type || box_type.empty?
  ...
  def preload_instances
    ...
    id_set.each_slice(1000) do |ids|
      @db[:ArchDescriptionInstances].filter(archDescriptionInstancesId: ids).each do |row|
        ...
        Log.info("Preloaded #{count} containers...") if (count % 100_000) == 0

        key = InstanceKey.from_row(row)

        next unless key # a digital object instance (or something else we don't care about)
        ...
        if data.empty?
          ...
          Log.error("DATA MISMATCH: #{key.inspect}: #{row.inspect}") if @key_set[key_bytes] && @key_set[key_bytes] != data.hash
