class Resource < Sequel::Model(:resource)
  ...
  def self.create_from_json(json, opts = {})
    ...
    sponsor[:finding_aid_sponsor_sha1] = Digest::SHA1.hexdigest(json.finding_aid_sponsor) if json.finding_aid_sponsor
  ...
  def self.ordered_record_properties(record_ids)
    ...
    filter(id: record_ids).select(:id, :level_id, :other_level).each do |row|
      id = row[:id]
      level = row[:other_level] || BackendEnumSource.value_for_id('archival_record_level', row[:level_id])
