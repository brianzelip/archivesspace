...
module Searchable
  ...
  ABSTRACT = ['abstract', 'scopecontent'].freeze
  ...
  def set_up_search(default_types = [], default_facets = [], default_search_opts = {}, params = {}, q = '')
    ...
    default_types = [limit] unless limit.blank?
    @query = ''
    ...
    if record_types
      ...
      pq = '*' if pq.blank?
      @query += "#{field}:" unless field.blank?
      @query += pq
    ...
    @base_search += "&limit=#{limit}" unless limit.blank?
  ...
  def set_up_advanced_search(default_types = [], default_facets = [], default_search_opts = {}, params = {})
    params = sanitize_params(params)
    @search = Search.new(params)
    default_types = @search[:limit].split(',') unless @search[:limit].blank?
    ...
    @search[:q].each_with_index { |query, i|
      ...
      builder.and('years', AdvancedQueryBuilder::RangeValue.new(from, to), 'range', false, op == 'NOT') unless from.blank? && to.blank?
  ...
  def process_search_results(base = '/search')
    ...
    unless @results['facets'].blank?
      @results['facets']['facet_fields'].keys.each do |type|
        facet_hash = strip_facets(@results['facets']['facet_fields'][type], 1, hits)
        next unless facet_hash.present?

        @facets[type] = facet_hash
        @facets['repository'].delete('global') if type == 'repository'
      end
    end
    @results = handle_results(@results, false)
    @repo = {}
    @repo = @results['results'][0]['_resolved_repository']['json'] || {} if !@results['results'].empty? && @results['results'][0]['_resolved_repository'].present?
    #    q = params.require(:q)
  ...
  def handle_results(results, _full = true)
    # FIXME: move facet handling to SolrResults
    results['facets']['facet_fields'] = strip_facet_fields(results['facets']['facet_fields']) unless results['facets'].blank? || results['facets']['facet_fields'].blank?
  ...
  def process_results(results, full)
    results.each do |result|
      ...
      if result['_resolved_repository'].is_a?(Hash)
        ...
        result['_resolved_repository']['json'] = ASUtils.json_parse(rr[1][0]['json']) unless rr[1][0]['json'].blank?
      end
      ...
      next unless result['_resolved_top_container_uri_u_sstr'].is_a?(Hash)

      # Pry::ColorPrinter.pp result['_resolved_top_container_uri_u_sstr']
      rr = result['_resolved_top_container_uri_u_sstr'].shift
      result['_resolved_top_container_uri_u_sstr']['json'] = ASUtils.json_parse(rr[1][0]['json']) unless rr[1][0]['json'].blank?
    end
    results
  end
  ...
  def search_terms(params)
    ...
    if queries
      ...
      queries.each_with_index do |query, i|
        ...
        next if from_year[i].blank? && to_year[i].blank?
  ...
  def default_search_opts(default = {})
    ...
    if AppConfig[:solr_params].any?
      AppConfig[:solr_params].each do |param, value|
        opts[param.to_sym] = if value.respond_to? :call
                               instance_eval(&value)
                             else
                               value
                             end
  ...
  def set_search_statement
    ...
    @search[:q].each_with_index do |q, i|
      condition += '<li>'
      if i == 0
        condition += I18n.t("search_results.op_first_row.#{@search[:op][i]}", default: '').downcase unless @search[:op][i].blank?
      else
  ...
  def full_title(json)
    ...
    ft = I18n.t('inherited', title: strip_mixed_content(json['title']), display: ft) unless json['title_inherited'].blank? || (json['display_string'] || '') == json['title']
    ft
  end
