    ...
    begin
      ...
      remaining.each do |table|
        foreign_key_list(table).each do |fk|
          ...
          greylist << fk[:table] if !greylist.include?(fk[:table]) && remaining.include?(fk[:table])
        end
      end

      remaining.each do |table|
        unless greylist.include?(table)
          puts "Dropping #{table}"
          drop_table?(table)
        end
      end
      ...
      ceiling -= 1
    end while !remaining.empty? && ceiling > 0
    unless remaining.empty?
      warn "Could not drop the following tables : #{remaining.join(',')}"
      warn '( check fk constaints )'
    end
  end
end
