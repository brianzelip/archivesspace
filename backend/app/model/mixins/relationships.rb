...
AbstractRelationship = Class.new(Sequel::Model) do
  ...
  def self.relate(obj1, obj2, properties)
    columns = if obj1.class == obj2.class
                ...
                # columns here anyway
                raise ReferenceError, "Can't relate an object to itself" if obj1.id == obj2.id

                reference_columns_for(obj1.class)
              else
                [reference_columns_for(obj1.class).first, reference_columns_for(obj2.class).first]
    end
    ...
    values.reject! { |key| (key == :id) || (key == 'id') }
    values.reject! { |key| (key == :jsonmodel_type) || (key == 'jsonmodel_type') } if obj1.is_a?(Location) || obj2.is_a?(Location)
    create(values)
  end
  ...
  def self.transfer(target, victims)
    target_columns = reference_columns_for(target.class)
    ...
    victims_by_model.each do |victim_model, victims|
      ...
      unless participating_models.include?(victim_model)
        found = find_by_participant_ids(victim_model, victims.map(&:id))

        raise ReferenceError, "#{victim_model} to be merged has data for relationship #{self}, but target record doesn't support it." unless found.empty?
      end

      victim_columns = reference_columns_for(victim_model)

      victim_columns.each do |victim_col|
        # Find any relationship where the current column contains a reference to
        # our victim
        filter(victim_col => victims.map(&:id)).each do |relationship|
          ...
          target_columns.each do |target_col|
            if relationship[target_col]
              ...
              raise 'Transfer would create a circular relationship!' if relationship[target_col] == target.id
  ...
  def self.values_for_property(obj, property)
    ...
    reference_columns_for(obj.class).each do |col|
      filter(col => obj.id).select(property).distinct.each do |relationship|
  ...
  def self.set_json_property(property)
    @json_property = property
  end

  class << self
    attr_reader :json_property
  end

  def self.set_participating_models(models)
    @participating_models = models
  end

  def self.participating_models
    @participating_models || raise('No participating models set')
  end

  def self.set_wants_array(val)
    @wants_array = val
  end

  def self.wants_array?
    @wants_array
  end
  ...
  def self.find_by_participant(obj)
    ...
    reference_columns = reference_columns_for(obj.class)
    matching_relationships = reference_columns.map { |col| filter(col => obj.id).all }.flatten(1)
    ...
    matching_relationships.select! { |relationship|
      our_columns.any? { |c|
  ...
  def self.find_by_participant_ids(participant_model, participant_ids)
    ...
    reference_columns = reference_columns_for(participant_model)

    reference_columns.each do |col|
      filter(col => participant_ids).each do |relationship|
        result << relationship
      end
    end
  ...
  def self.find_by_participants(objs)
    ...
    reference_columns = reference_columns_for(objs.first.class)

    objects_by_id = objs.group_by { |obj| obj.id }

    reference_columns.each do |col|
      eager(associations).filter(col => objects_by_id.keys).all.each do |relationship|
    ...
    result.each do |_obj, relationships|
  ...
  def self.who_participates_with(obj)
    ...
    relationships = find_by_participant(obj)
  ...
  def self.my_jsonmodel(ok_if_missing = false)
    raise("No corresponding JSONModel set for model #{inspect}") unless ok_if_missing
  end
...
module Relationships
  ...
  def assimilate(victims)
    victims = victims.reject { |v| (v.class == self.class) && (v.id == id) }
    ...
    DB.attempt {
      ...
      raise MergeRequestFailed, "Can't complete merge: record still in use"
    }
  ...
  def transfer_to_repository(repository, transfer_group = [])
    ...
    predicate = proc { |relationship|
      ...
      (referent.class.model_scope == :repository &&
       ...
       transfer_group.none? { |obj| obj.id == referent.id && obj.model == referent.model })
    }
  ...
  module ClassMethods
    ...
    def find_relationship(name, noerror = false)
      @relationships[name] || (noerror ? nil : raise("Couldn't find #{name} in #{@relationships.inspect}"))
    end

    # Define a new relationship.
    def define_relationship(opts)
      [:name, :contains_references_to_types].each do |p|
        opts[p] || raise("No #{p} given")
      end
      ...
      ArchivesSpaceService.loaded_hook do
        ...
        clz = Class.new(AbstractRelationship) do
          ...
          Log.warn("Table doesn't exist: #{table_name}") unless db.table_exists?(table_name)
        ...
        Relationships.const_set(name + opts[:name].to_s.camelize, clz)
      end
    end
    ...
    def delete_existing_relationships(obj, bump_lock_version_on_referent = false, force = false, predicate = nil)
      relationships.each do |relationship_defn|
        ...
        if relationship_defn.json_property &&
           (!my_jsonmodel.schema['properties'][relationship_defn.json_property] ||
            my_jsonmodel.schema['properties'][relationship_defn.json_property]['readonly'] === 'true')
    ...
    def apply_relationships(obj, json, opts, new_record = false)
      delete_existing_relationships(obj) unless new_record
      ...
      @relationships.each do |relationship_name, relationship_defn|
        ...
        ASUtils.as_array(json[property_name]).each_with_index do |reference, idx|
          ...
          }) || raise("Couldn't find model for #{record_type[:type]}")

          referent = referent_model[record_type[:id]]

          raise ReferenceError, "Can't relate to non-existent record: #{reference['ref']}" unless referent
          ...
          DB.increase_lock_version_or_fail(referent) if referent_model.find_relationship(relationship_name, true) && !opts[:system_generated]
        end
      end
    end
    ...
    def sequel_to_jsonmodel(objs, opts = {})
      ...
      jsons.zip(objs).each do |json, obj|
        relationships.each do |relationship_defn|
          ...
          json[property_name] = json[property_name].first unless relationship_defn.wants_array?
        end
      end
      ...
      relationships.map do |relationship_defn|
        ...
        their_ref_columns.each do |their_col|
          my_ref_columns.each do |my_col|
            ...
            base_ds = join(relationship_defn,
                           Sequel.qualify(relationship_defn.table_name, my_col) =>
                                  Sequel.qualify(table_name, :id))
            ...
            # relationship.
            update_toplevel_mtimes(base_ds, now)
          end
        end
      end
    end
    ...
    def update_toplevel_mtimes(dataset, new_mtime)
      if enclosing_associations.empty?
        # If we're not enclosed by anything else, we're a top-level record.  Do the final update.
        if DB.supports_join_updates?
          # Fast path!  Use a join update.
          dataset.update(Sequel.qualify(table_name, :system_mtime) => new_mtime)
        else
          # Slow path.  Subselect.
          ids_to_touch = dataset.select(Sequel.qualify(table_name, :id))
          filter(id: ids_to_touch).update(system_mtime: new_mtime)
        end
      else
        # Otherwise, we're a nested record
        enclosing_associations.each do |association|
          parent_model = association[:model]

          # Link the parent into the current dataset
          parent_ds = dataset.join(parent_model,
                                   Sequel.qualify(table_name, association[:key]) =>
