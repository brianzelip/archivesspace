  ...
  def self.validate(current_schema, data, fragments, validator, options = {})
    ...
    if data.is_a?(Hash)
      current_schema.schema['properties'].each do |property, property_schema|
        next unless property_schema['ifmissing'] && !data.has_key?(property)
        ...
        validation_error(message, fragments, current_schema, self, options[:record_errors]) if message
      end
    end
  end
end
...
class ArchivesSpaceTypeAttribute < JSON::Schema::TypeAttribute
  ...
  def self.validation_error_for(expected_type, fragments, current_schema)
    ...
    unless Thread.current[:json_validation_cached_errors][expected_type]
  ...
  def self.validate(current_schema, data, fragments, validator, options = {})
    ...
    if types == 'object' && data.is_a?(Hash) && data.has_key?('ref') && current_schema.schema['subtype'] != 'ref'
      ...
      warn("WARNING: Schema #{current_schema.inspect} appears to be missing a subtype definition of 'ref'")
    end
...
class ArchivesSpaceSubTypeAttribute < JSON::Schema::TypeAttribute
  def self.validate(current_schema, data, fragments, _validator, options = {})
...
class ArchivesSpaceDynamicEnumAttribute < JSON::Schema::TypeAttribute
  def self.validate(current_schema, data, fragments, _validator, options = {})
    ...
    unless JSONModel.init_args[:enum_source].valid?(enum_name, data)
      ...
      klass = if enum_source.respond_to?(:editable?) &&
