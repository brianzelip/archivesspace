    ...
    jsons.zip(objs).each do |json, obj|
      ...
      relationships.each do |relationship|
        ...
        instances.each do |link|
          uri = uri_for(:resource, link[:resource_id]) if link[:resource_id]
          uri = uri_for(:archival_object, link[:archival_object_id]) if link[:archival_object_id]
          uri = uri_for(:accession, link[:accession_id]) if link[:accession_id]

          raise 'Digital Object Instance not linked to either a resource, archival object or accession' if uri.nil?
