class RepositoriesController < ApplicationController
  ...
  def generate_names(repository_with_agent)
    ...
    repository_with_agent['agent_representation']['agent_contacts']['0']['name'] = name['primary_name'] if repository_with_agent['agent_representation']['agent_contacts']['0']['name'].blank?
  end
  ...
  def create
    ...
    handle_crud(instance: :repository,
                ...
                on_invalid: lambda {
                  @exceptions[:errors]['repository/repo_code'] = @exceptions[:errors].delete('repo_code') if @exceptions[:errors]['repo_code']
  ...
  def handle_repository_oai_params(params)
    ...
    repo_params_hash['oai_sets_available'] = if form_oai_sets_hash
                                               form_oai_sets_hash.keys.to_json
                                             else
                                               '[]'
                                             end

    # handle oai toggle flag
    repo_params_hash['oai_is_disabled'] = if form_oai_enabled_hash
                                            form_oai_enabled_hash['oai_is_disabled']
                                          else
                                            0
                                          end
  end
