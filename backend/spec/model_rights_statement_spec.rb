...
describe 'Rights Statement model' do
  ...
  it 'Supports creating a new rights statement' do
    ...
    expect(RightsStatement[rights_statement[:id]].identifier).to eq(opts[:identifier])
  end
  ...
  it 'Enforces validation rules when rights_type is copyright' do
    ...
    expect { create_rights_statement(opts) }.not_to raise_error
  end
  ...
  it 'Allows a rights statement to be created with an external document' do
    ...
    expect(RightsStatement[rights_statement[:id]].external_document.length).to eq(1)
    expect(RightsStatement[rights_statement[:id]].external_document[0].title).to eq(opts['external_documents'][0]['title'])
  ...
  it 'Allows a rights statement to be created with a linked agent' do
    ...
    expect(RightsStatement.to_jsonmodel(rights_statement[:id]).linked_agents.length).to eq(1)
    expect(RightsStatement.to_jsonmodel(rights_statement[:id]).linked_agents[0]['ref']).to eq(agent.uri)
  end
  ...
  it 'Allows a rights statement to be created with a note' do
    ...
    expect(RightsStatement.to_jsonmodel(rights_statement[:id]).notes.length).to eq(1)
  end
  ...
  it 'Allows a rights statement to be created with an act' do
    ...
    expect(RightsStatement.to_jsonmodel(rights_statement[:id]).acts.length).to eq(1)
  end

  it 'applies validation rules to act' do
    expect {
      create_rights_statement(
  ...
  it 'will generate a ref_id if non is provided' do
    ...
    expect(RightsStatement[rights_statement[:id]].identifier).not_to be_nil
  end
end
