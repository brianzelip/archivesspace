class Record
  ...
  def initialize(solr_result, full = false)
    @raw = solr_result
    @json = if solr_result['json'].is_a? Hash
              solr_result['json']
            else
              ASUtils.json_parse(solr_result['json']) || {}
            end
  ...
  def parse_full_title
    ...
    ft = I18n.t('inherited', title: process_mixed_content(json['title'], preserve_newlines: true), display: ft) unless json['title_inherited'].blank? || (json['display_string'] || '') == json['title']
    ft
  end

  def parse_identifier
    json.dig('_composite_identifier') || json.dig('component_id') ||
      ([json.dig('id_0'), json.dig('id_1'), json.dig('id_2'), json.dig('id_3')].select { |x| !x.nil? && !x.empty? }).join('-')
  end
  ...
  def parse_container_display(opts = {})
    ...
    if !json['instances'].blank? && json['instances'].is_a?(Array)
      json['instances'].each do |inst|
        ...
        containers << if include_uri && top_container_uri = sub_container.dig('top_container', 'ref')
                        {
                          'title' => container_display_string,
                          'uri' => top_container_uri
                        }
                      else
                        container_display_string
                      end
      ...
      notes.each do |k, v|
        if k == 'content' || k == 'items'
          ASUtils.wrap(v).each do |s|
            if s.is_a? String
              s.gsub!(%r{<ref .*?target="(.+?)".*?>(.+?)</ref>}m, "<a href='#{app_prefix(base_uri)}/resolve/\\1'>\\2</a>")
            else
  ...
  def parse_lang_materials
    ...
    json['lang_materials'].each do |lang_material|
      next if lang_material['language_and_script'].blank?
  ...
  def parse_repository
    if raw['_resolved_repository'].is_a?(Hash)
      ...
      return ASUtils.json_parse(rr[1][0]['json']) unless rr[1][0]['json'].blank?
  ...
  def parse_agents(subjects_arr)
    ...
    ASUtils.wrap(json['linked_agents']).each do |relationship|
      next if relationship['role'].blank? || relationship['_resolved'].blank?
  ...
  def parse_repository_info
    ...
    unless resolved_repository.nil?
      ['name', 'uri', 'url', 'parent_institution_name', 'image_url', 'repo_code'].each do |item|
      ...
      unless resolved_repository['agent_representation'].blank? || resolved_repository['agent_representation']['_resolved'].blank? || resolved_repository['agent_representation']['_resolved']['jsonmodel_type'] != 'agent_corporate_entity'
        ...
        ['city', 'region', 'post_code', 'country', 'email'].each do |k|
        ...
        info['telephones'] = in_h['telephones'] unless in_h['telephones'].blank?
      end
    end
    info
  end
  ...
  def cite_url_and_timestamp
    "#{AppConfig[:public_proxy_url].sub(%r{^/}, '')}#{uri}  #{I18n.t('accessed')}  #{Time.now.strftime('%B %d, %Y')}"
  end
  ...
  def parse_digital_object_instances
    ...
    ASUtils.wrap(json['instances']).each do |instance|
      next unless instance['digital_object'] && instance['digital_object']['ref']
  ...
  def build_request_item_container_info
    ...
    [:top_container_url, :container, :location_title, :location_url, :machine, :barcode].each { |sym| container_info[sym] = [] }
