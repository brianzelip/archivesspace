class Resource < JSONModel(:resource)
  def initialize(values)
    ...
    self.extents = [JSONModel(:extent).new._always_valid!] if !extents || extents.empty?

    self.dates = [JSONModel(:date).new._always_valid!] if !dates || dates.empty?

    self.lang_materials = [JSONModel(:lang_material).new._always_valid!] if !lang_materials || lang_materials.empty?
  ...
  def populate_from_accession(accession)
    ...
    values = JSONSchemaUtils.map_hash_with_schema(values, JSONModel(:accession).schema,
                                                  [proc { |hash, _schema|
                                                    hash = hash.clone
                                                    hash.delete_if { |k, _v| k.to_s =~ /^(id_[0-9]|lock_version|instances|deaccessions|collection_management|user_defined|external_documents)$/ }
                                                    hash
                                                  }])
