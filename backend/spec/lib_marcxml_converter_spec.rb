...
describe 'MARCXML converter' do
  ...
  describe 'Basic MARCXML to ASPACE mappings' do
    ...
    it "maps field 630 to resource['subjects']" do
      expect(@subjects[1]['terms'].map { |t| t['term_type'] }.sort).to eq(['uniform_title', 'topical'].sort)
    end
      ...
      def convert_test_file
        ...
        @people.instance_eval do
          def by_name(name)
            self.select { |p|
              if name.match(/(.+),\s*(.+)/)
                (p['names'][0]['primary_name'] == Regexp.last_match(1)) && (p['names'][0]['rest_of_name'] == Regexp.last_match(2))
              else
  ...
  describe 'Name Order handling' do
    ...
    it 'imports name_person subrecords with the correct name_order' do
      expect(@names.map { |name| name['name_order'] }).to eq(['inverted', 'direct', 'inverted', 'direct', 'inverted', 'direct'])
    end
  ...
  describe 'Subclassing and reconfiguring' do
    ...
    let (:subclass) {
      class CramXMLConverter < MarcXMLConverter
        def self.import_types(*_args)
          { name: 'cramxml', description: 'cram records in' }
        end

        def self.instance_for(type, input_file)
          new(input_file) if type == 'cramxml'
        end
      end

      CramXMLConverter.configure do |config|
        config['/record'][:map]['self::record'] = proc { |resource, _node|
          resource.title = 'TITLE' unless resource.title

          resource.dates << ASpaceImport::JSONModel(:date).from_hash(:expression => '1945', :label => 'creation', 'date_type' => 'single') if resource.dates.nil? || resource.dates.empty?
          ...
          resource.extents << ASpaceImport::JSONModel(:extent).from_hash(portion: 'whole', number: '1', extent_type: 'linear_feet') if resource.extents.nil? || resource.extents.empty?

          resource.id_0 = 'ID' if resource.id_0.nil? || resource.id.empty?
        }
      end
  ...
  describe 'Handling bad mappings' do
    ...
    let (:subconverter) {
      class BadMarcXMLAccessionConverter < MarcXMLConverter
        ...
        def self.instance_for(type, input_file)
          new(input_file) if type == 'marc2accession'
        end
      end
      ...
      BadMarcXMLAccessionConverter.configure do |config|
        config['/record'] = {
          obj: :accession,
          map: {
            'self::record' => proc { |accession, _node|
              accession.title = 'TITLE' unless accession.title

              accession.extents << ASpaceImport::JSONModel(:extent).from_hash(portion: 'whole', number: '1', extent_type: 'linear_feet') if accession.extents.nil? || accession.extents.empty?

              accession.id_0 = 'ID' if accession.id_0.nil? || accession.id.empty?
            },
            'datafield[@tag="9999"]' => {
              obj: :note_singlepart,
              rel: :notes, # Accessions don't take notes!
              map: {
                'self::datafield' => lambda { |note, _node|
