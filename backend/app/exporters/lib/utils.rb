...
module ASpaceExport
  module Utils
    ...
    def self.extract_note_text(note, include_unpublished = false, add_punct = false)
      ...
      note_text << '.' if add_punct == true && !note_text.empty? && !['.', '!', '?'].include?(note_text[-1])
    ...
    def self.include_p?(note_type)
      notes_with_p.include?(note_type)
    end

    def self.headless_note?(note_type, content)
      if content.strip.start_with?('<head') || headless_notes.include?(note_type)
        true
      else
        false
      end
