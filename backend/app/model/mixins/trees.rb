module Trees
  ...
  def children
    self.class.node_model
        .this_repo.filter(root_record_id: id,
                          parent_id: nil)
  end
  ...
  def build_node_query
    self.class.node_model.this_repo.filter(root_record_id: id)
  end
  ...
  def tree(ids_of_interest = :all, display_mode = :full)
    ...
    loop do
      ...
      nodes.each do |node|
        ...
        properties[node.id]['has_children'] = !!has_children[node.id] if ids_of_interest != :all

        load_node_properties(node, properties, ids_of_interest) unless display_mode == :sparse
      end
    ...
    result = {
      ...
      publish: respond_to?(:publish) ? publish === 1 : true,
      suppressed: respond_to?(:suppressed) ? suppressed === 1 : false,
      children: top_nodes.sort_by(&:first).map { |_position, node| self.class.assemble_tree(node, links, properties) },
      record_uri: self.class.uri_for(root_type, id)
    }

    unless display_mode == :sparse
      result[:finding_aid_filing_title] = finding_aid_filing_title if respond_to?(:finding_aid_filing_title) && !finding_aid_filing_title.nil? && !finding_aid_filing_title.empty?
  ...
  def ordered_records
    ...
    self.class.node_model
      ...
      excluded_rows[row[:id]] = true if row[:publish] == 0 || row[:suppressed] == 1
    end
    ...
    until root_set.empty?
      next_rec = root_set.shift
      if next_rec.nil?
        # Our first iteration.  Nothing to add yet.
      else
        result << next_rec unless excluded_rows[next_rec]
      end
    ...
    extra_root_properties = self.class.ordered_record_properties([id])
    extra_node_properties = self.class.node_model.ordered_record_properties(result)

    [{ 'ref' => uri,
       'display_string' => title,
       'depth' => 0 }.merge(extra_root_properties.fetch(id, {}))] +
      result.map { |id|
        {
  ...
  module ClassMethods
    ...
    def assemble_tree(node, links, properties)
      ...
      result['has_children'] = !!links[node] unless result.has_key?('has_children')

      result['children'] = if links[node]
                             links[node].sort_by(&:first).map do |_position, child_id|
