class OaiConfigController < ApplicationController
  ...
  def handle_oai_config_params(params)
    ...
    oai_config_hash['repo_set_codes'] = if repo_set_codes_hash
                                          params['repo_set_codes'].keys.to_json
                                        else
                                          '[]'
                                        end
    ...
    oai_config_hash['sponsor_set_names'] = if oai_config_hash['sponsor_set_names']
                                             oai_config_hash['sponsor_set_names'].split('|').to_json
                                           else
                                             '[]'
                                           end
  end
