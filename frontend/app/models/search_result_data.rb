class SearchResultData
  ...
  def filtered_terms?
    @search_data[:criteria].has_key?('filter_term[]') && !@search_data[:criteria]['filter_term[]'].reject { |f| f.empty? }.empty?
  end
  ...
  def facet_label_for_filter(filter)
    ...
    if @facet_data.has_key?(facet) && @facet_data[facet].has_key?(term)
  ...
  def facets_for_filter
    ...
    facet_data_for_filter.each { |_facet_group, facets|
      facets.delete_if { |_facet, facet_map|
        facet_map[:count] === @search_data['total_hits']
      }
    }
    facet_data_for_filter.delete_if { |_facet_group, facets| facets.empty? }
  ...
  def facet_label_string(facet_group, facet)
    ...
    return I18n.t("boolean.#{facet}", default: facet) if facet_group === 'publish'
    return I18n.t("enumerations.digital_object_digital_object_type.#{facet}", default: facet) if facet_group === 'digital_object_type'
    return I18n.t("enumerations.location_temporary.#{facet}", default: facet) if facet_group === 'temporary'
    return I18n.t("enumerations.event_event_type.#{facet}", default: facet) if facet_group === 'event_type'
    return I18n.t("enumerations.event_outcome.#{facet}", default: facet) if facet_group === 'outcome'
    return I18n.t("enumerations.subject_term_type.#{facet}", default: facet) if facet_group === 'first_term_type'

    if facet_group === 'source'
      if single_type? && (types[0] === 'subject')
    ...
    if facet_group === 'level'
      if single_type? && (types[0] === 'digital_object')
        return I18n.t("enumerations.digital_object_level.#{facet}", default: facet)
      else
        return I18n.t("enumerations.archival_record_level.#{facet}", default: facet)
      end
    end
    ...
    return ClassificationHelper.format_classification(ASUtils.json_parse(facet)) if facet_group === 'classification_path'

    return I18n.t("assessment._frontend.assessment_review_required.#{facet}_value") if facet_group === 'assessment_review_required'

    return I18n.t("assessment._frontend.assessment_sensitive_material.#{facet}_value") if facet_group === 'assessment_sensitive_material'

    return I18n.t("assessment._frontend.assessment_inactive.#{facet}_value") if facet_group === 'assessment_inactive'

    return I18n.t("#{facet}._singular", default: facet) if facet_group === 'assessment_record_types'

    return I18n.t("assessment._frontend.assessment_completed.#{facet}_value") if facet_group === 'assessment_completed'

    facet
  end

  def results?
    @search_data.has_key?('results') && !@search_data['results'].empty?
  end

  def has_titles?
    if @search_data[:criteria].has_key?('type[]') && (types - self.class.UNTITLED_TYPES).empty?
      false
    else
      true
    end
  ...
  def sorted_by(index = 0)
    @sort_data[index][:field] if sorted? && @sort_data[index]
  end
