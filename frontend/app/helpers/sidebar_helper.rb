  ...
  class SidebarGenerator
    ...
    def ensure_properties(opts, properties)
      properties.each do |p|
        raise "Missing required property: #{p}" unless opts[p]
      end
    end
    ...
    def render_for_view_and_edit(opts)
      ...
      render_entry(opts) if @form.controller.action_name != 'show' || property == :none || !record[property].blank?
    end
    ...
    def render_for_view_only(opts)
      ...
      render_entry(opts) if @form.controller.action_name == 'show' && (!record[property].blank? || property == :none)
    end
