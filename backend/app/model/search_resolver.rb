class SearchResolver
  ...
  def resolve(results)
    @resolve_definitions.each do |resolve_def|
      ...
      Log.warn("Resolve query hit MAX_RESOLVE_RESULTS.  Result set may be incomplete: #{query.to_hash.inspect}") if resolved_results['total_hits'] > MAX_RESOLVE_RESULTS
      ...
      results['results'].each do |result|
        resolved = resolved_results['results'].map { |resolved|
          ...
          { key => [SearchResolver.resolver_for(custom_resolver).resolve(resolved)] } if ASUtils.wrap(result[source_field]).include?(key)
        }.compact

        # Merge our entries into a single hash keyed on `key`
        result["_resolved_#{source_field}"] = resolved.reduce { |merged, elt| merged.merge(elt) { |_key, coll1, coll2| coll1 + coll2 } }
      end
    end
  end
  ...
  ResolveDefinition = Struct.new(:source_field, :target_field, :custom_resolver) do
    def self.parse(resolve_def)
      ...
      raise "Resolve request parameter not well-formed: #{resolve_def}.  Should be source_field:target_field" unless source_field && target_field
