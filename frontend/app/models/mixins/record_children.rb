module RecordChildren
  ...
  module ClassMethods
    def child_type
      JSONModel.parse_jsonmodel_ref(schema['properties']['children']['items']['type']).first
    end
    ...
    def clean_dates(child)
      if child['dates']
        if child['dates'][0].reject { |_k, v| v.blank? }.empty?
          child.delete('dates')
    ...
    def clean_notes(child)
      if child['notes']
        (0..2).each do |i|
          if !child['notes'][i]['type'].blank?
            note_types_for_child.each do |notetype|
              child['notes'][i]['jsonmodel_type'] = notetype.to_s if JSONModel.enum_values(JSONModel(notetype).schema['properties']['type']['dynamic_enum']).include?(child['notes'][i]['type'])
            end
          ...
          elsif child['notes'][i]['type'].blank? && child['notes'][i]['content'][0].blank?
            child['notes'][i] = nil
          end
        end
        child['notes'].compact!
