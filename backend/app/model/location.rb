class Location < Sequel::Model(:location)
  ...
  def self.create_for_batch(batch)
    ...
    locations.map { |location| create_from_json(location) }
  end
  ...
  def self.titles_for_batch(batch)
    locations = generate_locations_for_batch(batch)
    locations.map { |location| generate_title(location) }
  end
  ...
  def delete
    ...
    raise ConflictException, 'Location cannot be deleted if linked' if object_graph.models.any? { |model| model.is_relationship? && !ignored_relationships.include?(model) }
  ...
  def self.building_data
    buildings = {}
    all = where('building IS NOT NULL').order_by(:building, :floor, :room, :area)
    all.each do |location|
      buildings[location.building] = {} unless buildings.has_key?(location.building)
