...
class ArchivesSpaceService < Sinatra::Base
  ...
  do
    ...
    if user
      ...
      raise NotFoundException, "User wasn't found"
    end
  end
  ...
  do
    ...
    raise AccessDeniedException unless current_user.can?(:manage_users)

    params[:user].username = Username.value(params[:user].username)

    user.update_from_json(params[:user])

    DBAuth.set_password(params[:user].username, params[:password]) if params[:password]
  ...
  do
    ...
    raise NotFoundException unless user
  ...
  def check_admin_access
    raise AccessDeniedException, 'Only admins can create admin users' if params[:user].is_admin && !current_user.can?(:administer_system)
