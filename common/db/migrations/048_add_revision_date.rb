...
Sequel.migration do
  up do
    ...
    self[:resource].filter(Sequel.~(finding_aid_revision_date: nil)).or(Sequel.~(finding_aid_revision_description: nil)).each do |row|
      date = row[:finding_aid_revision_date] || 'finding aid revision date not supplied'
      desc = row[:finding_aid_revision_description] || 'finding aid revisiion description not supplied'
