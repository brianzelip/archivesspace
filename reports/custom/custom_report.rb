class CustomReport < AbstractReport
  ...
  def initialize(params, job, db)
    ...
    @conditions = if @possible_fields.include? 'repo_id'
                    ["repo_id = #{@repo_id} or repo_id is null"]
                  else
                    ['1 = 1']
                  end
    ...
    CustomField.fields_for(@record_type).each do |field|
      ...
      next unless ASUtils.present?(template['fields'][field_name]['narrow_by']) &&
