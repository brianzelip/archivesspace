class User < Sequel::Model(:user)
  ...
  def self.make_admin_if_requested(obj, json)
    return unless RequestContext.get(:apply_admin_access)
  ...
  def self.sequel_to_jsonmodel(objs, opts = {})
    ...
    jsons.zip(objs).each do |json, obj|
      json['agent_record'] = { 'ref' => uri_for(obj.agent_record_type, obj.agent_record_id) } if obj.agent_record_id

      json['is_admin'] = true if obj.can?(:administer_system)
    end
    jsons
  end
  ...
  def can?(permission_code, _opts = {})
    return true if derived_permissions.include?(permission_code.to_s)
  ...
  def permissions
    ...
    ds.each do |row|
      ...
      global_permissions << row[:permission_code] if row[:repo_code] == Repository.GLOBAL
    end
  ...
  def add_to_groups(groups, delete_all_for_repo_id = false)
    if delete_all_for_repo_id
      ...
      self.class.db[:group_user].where(user_id: id, group_id: groups_ids).delete
    end
  ...
  def delete
    raise AccessDeniedException, "Can't delete system user" if is_system_user == 1

    DBAuth.delete_user(username)

    # transfer all import jobs to the admin user
    admin_user = User.select(:id).where(username: 'admin').first
    Job.filter(owner_id: id).update(owner_id: admin_user.id)

    Preference.filter(user_id: id).delete
    remove_all_group

    super

    AgentPerson[agent_record_id].delete if agent_record_id
  end
end
