...
def calculate_hash(db, table, row)
  ...
  db[:date].filter(foreign_key => row[:id]).each do |date|
    hash_fields << ['date_type_id', 'label_id', 'certainty_id', 'expression', 'begin', 'end', 'era_id', 'calendar_id'].map { |field|
      date[field.intern] || ' '
    }.join('_')
  end

  db[:agent_contact].filter(foreign_key => row[:id]).each do |contact|
    hash_fields << ['name', 'salutation_id', 'telephone', 'address_1', 'address_2', 'address_3', 'city', 'region', 'country', 'post_code', 'telephone_ext', 'fax', 'email', 'email_signature', 'note'].map { |field|
      contact[field.intern] || ' '
    }.join('_')
  end

  db[:external_document].filter(foreign_key => row[:id]).each do |doc|
    hash_fields << ['title', 'location'].map { |field|
      doc[field.intern] || ' '
    }.join('_')
  end
  ...
  name_fields = ['dates', 'qualifier', 'source_id', 'rules_id']
  name_fields += case name_table
                 when :name_person
                   ['primary_name', 'name_order.id', 'prefix', 'rest_of_name', 'suffix', 'fuller_form', 'number']
                 when :name_family
                   ['family_name', 'prefix']
...
Sequel.migration do
  AGENT_TABLES = [:agent_person, :agent_corporate_entity, :agent_family, :agent_software].freeze
      ...
      self[table].each_by_page do |row|
        ...
        if used_hashes.include?(digest)
          ...
          [:created_by, :last_modified_by, :create_time, :system_mtime, :user_mtime].each do |col|
            values[col] = row[col] if row.has_key?(col)
          end
          ...
          # recalculate
          digest = calculate_hash(self, table, row)
          raise Sequel::UniqueConstraintViolation, 'Gave up trying to create unique hashes for existing agents' if used_hashes.include?(digest)
        end
