...
class IndexerCommon
  ...
  def self.extract_string_values(doc)
    text = ''
    doc.each do |key, val|
      if ['created_by', 'last_modified_by', 'system_mtime', 'user_mtime', 'json', 'types', 'create_time', 'date_type', 'jsonmodel_type', 'publish', 'extent_type', 'language', 'script', 'system_generated', 'suppressed', 'source', 'rules', 'name_order'].include?(key)
      elsif key =~ /_enum_s$/
      elsif val.is_a?(String)
        text << "#{val} "
      elsif val.is_a?(Hash)
        text << extract_string_values(val)
      elsif val.is_a?(Array)
        val.each do |v|
          if v.is_a?(String)
            text << "#{v} "
          elsif v.is_a?(Hash)
            text << extract_string_values(v)
          end
        end
      end
    end
  ...
  def self.build_fullrecord(record)
    ...
    ['finding_aid_subtitle', 'finding_aid_author'].each do |field|
      fullrecord << "#{record['record'][field]} " if record['record'].has_key?(field)
    end
  ...
  def add_agents(doc, record)
    if record['record']['linked_agents']
      ...
      doc['creators'] = creators.collect { |link| link['_resolved']['display_name']['sort_name'] } unless creators.empty?
      ...
      record['record']['linked_agents'].each do |link|
        if seen[link['ref']] == 'creator'
          ...
          doc["#{link['ref'].gsub(%r{/}, '_')}_relator_sort"] = "#{link['role']} #{relator_label}"
  ...
  def add_subjects(doc, record)
    doc['subjects'] = record['record']['subjects'].map { |s| s['_resolved']['title'] }.compact if record['record']['subjects']
  end
  ...
  def add_notes(doc, record)
    doc['notes'] = record['record']['notes'].map { |note| IndexerCommon.extract_string_values(note) }.join(' ') if record['record']['notes']
  end
  ...
  def configure_doc_rules
    add_document_prepare_hook { |doc, record|
      ...
      ASUtils.search_nested(record['record'], ['items'], ['_resolved']) do |_field, field_value|
    ...
    add_document_prepare_hook { |doc, record|
      if doc['primary_type'] == 'location'
        doc['temporary'] = record['record']['temporary'] if record['record'].has_key? 'temporary'
    ...
    add_document_prepare_hook { |doc, _record|
      doc['repository'] = doc['id'] if doc['primary_type'] == 'repository'
    }
    ...
    add_document_prepare_hook { |doc, record|
      if ['classification', 'classification_term'].include?(doc['primary_type'])
        ...
        doc['published_agent_uris'] = []
        unless record.dig(:record, :creator, :_resolved).nil?
          doc['published_agent_uris'] << record['record']['creator']['ref'] if record['record']['creator']['_resolved']['publish'] && !record['record']['creator']['ref'].nil?
        end
    ...
    add_document_prepare_hook { |doc, record|
      doc['classification'] = record['record']['classification']['ref'] if doc['primary_type'] == 'classification_term'
    }

    add_document_prepare_hook { |doc, record|
      records_with_classifications = ['resource', 'accession']

      if records_with_classifications.include?(doc['primary_type']) && !record['record']['classifications'].empty?
    ...
    add_document_prepare_hook { |doc, record|
      if ['resource', 'archival_object', 'accession'].include?(doc['primary_type']) && record['record']['instances'] && !record['record']['instances'].empty?
    ...
    add_document_prepare_hook { |doc, record|
      ...
      doc['four_part_id'] = four_part_id unless four_part_id.empty?
    }
    ...
    add_document_prepare_hook { |doc, record|
      if record['record']['jsonmodel_type'] == 'top_container'
        ...
        unless record['record']['container_locations'].empty?
          record['record']['container_locations'].each do |container_location|
            next unless container_location['status'] == 'current'
    ...
    add_document_prepare_hook { |doc, record|
      if ['resource', 'archival_object', 'accession'].include?(doc['primary_type'])
        ...
        ASUtils.wrap(record['record']['instances']).each { |instance|
          next unless instance['sub_container'] && instance['sub_container']['top_container']
    ...
    add_document_prepare_hook { |doc, record|
      if doc['primary_type'] == 'assessment'
        ...
        unless ASUtils.wrap(record['record']['reviewer']).empty?
  ...
  def delete_records(records, opts = {})
    ...
    delete_request = { delete: records.map { |id|
                                 [{ 'id' => id },
                                  { 'query' => opts.fetch(:parent_id_field, 'parent_id') + ":\"#{id}\"" }]
                               } .flatten(1) }
  ...
  def index_records(records, timing = IndexerTiming.new)
    ...
    timing.time_block(:conversion_ms) do
      records.each do |record|
        ...
        next if !record_type || skip_index_record?(record) || (record_type != 'repository' && !record_types.include?(record_type.intern))
        ...
        doc['title'] = if !values['finding_aid_filing_title'].nil? && !values['finding_aid_filing_title'].empty?
                         values['finding_aid_filing_title']
                       else
                         values['title']
                       end
        ...
        doc['publish'] = if doc['suppressed']
                           ...
                           values.has_key?('publish') && values['publish']
                         end
  ...
  def index_batch(batch, timing = IndexerTiming.new, opts = {})
    ...
    unless batch.empty?
      ...
      records_with_children = batch.map { |e|
        "\"#{e['id']}\"" if self.records_with_children.include?(e['primary_type'].to_s)
      }.compact

      unless records_with_children.empty?
        req = Net::HTTP::Post.new("#{solr_url.path}/update")
        req['Content-Type'] = 'application/json'
      ...
      timing.time_block(:solr_add_ms) do
        ...
        Log.error "SolrIndexerError when indexing records: #{response.body}" if response.code != '200'
      end
    end
  end
  ...
  def paused?
    singleton_class.class_variable_get(:@@paused_until) > Time.now
  end

  def skip_index_record?(_record)
    false
  end

  def skip_index_doc?(_doc)
    false
  end
  ...
  def apply_pui_fields(doc, record)
    ...
    if doc['publish']
      ...
      doc['types'] << 'pui_record' if object_record_types.include?(doc['primary_type'])
      ...
      unless RecordInheritance.has_type?(doc['primary_type'])
        ...
        doc['types'] << if ['resource'].include?(doc['primary_type'])
                          'pui_collection'
