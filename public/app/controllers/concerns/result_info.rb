module ResultInfo
  ...
  def breadcrumb_info
    ...
    unless !path || !path.is_a?(Array) || path.empty?
  ...
  def handle_dates(json)
    json['html'] = {} unless json.dig('html')
    json['html']['dates'] = []
  ...
  def process_repo_info(repo)
    ...
    info['top'] = {}
    unless repo.nil?
      ['name', 'uri', 'url', 'parent_institution_name', 'image_url', 'repo_code', 'description'].each do |item|
  ...
  def process_agents(agents_arr, subjects_arr = [])
    ...
    agents_arr.each do |agent|
      next if agent['role'].blank? || agent['_resolved'].blank?
  ...
  def process_digital(json)
    dig_obj = {}
    dig_obj['out'] = json['digital_object_id'] unless json['digital_object_id'].blank? || !json['digital_object_id'].start_with?('http')
  ...
  def process_digital_instance(instances)
    ...
    if instances&.is_a?(Array)
      instances.each do |instance|
        if instance.dig('digital_object', '_resolved')
          dig_f = {}
      ...
      json['file_versions'].each do |version|
        ...
        if version.dig('publish') != false && version['file_uri'].start_with?('http')
          if version.dig('xlink_show_attribute') == 'embed'
            ...
            rep_caption = if version['caption'].blank?
                            version['title']
                          else
  ...
  def process_subjects(subjects_arr)
    ...
    subjects_arr.each do |subject|
      return_arr.push(subject['_resolved']) unless subject['_resolved'].blank?
    end
    return_arr
  end
  ...
  def get_rep_image(instances)
    ...
    if instances&.is_a?(Array)
      instances.each do |instance|
        next if instance['digital_object'].blank? || instance['digital_object']['_resolved'].blank?

        it = instance['digital_object']['_resolved']
        next if !it['publish'] || it['file_versions'].blank?
