    ...
    unless params[:children].empty?
      ...
      if target_class == child_class
        ...
        loop do
          ...
          raise ConflictException, "Can't make a parent into its own child" if params[:children].include?(ancestor.uri)
        end
      end
      ...
      ordered = if (target.id == first_obj.parent_id || (target.class.included_modules.include?(Trees) && first_obj.parent_id.nil?)) && first_obj.logical_position < position
