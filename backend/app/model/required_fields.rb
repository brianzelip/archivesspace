class RequiredFields < Sequel::Model(:required_fields)
  ...
  def self.create_or_update(json, repo_id, record_type)
    ...
    if self[id]
      ...
      create_from_json(json, id: "#{repo_id}_#{record_type}")
    end
  ...
  def update_from_json(json, opts = {}, _apply_nested_records = false)
    json['lock_version'] ||= 0
    super(json, opts.merge('blob' => json.to_json))
  end

  def uri
    "/repositories/#{repo_id}/required_fields/#{record_type}"
  end
end
