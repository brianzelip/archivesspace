    ...
    module ClassMethods
      ...
      def prepare_for_db(jsonmodel_class, hash)
        ...
        schema['properties'].each do |property, definition|
          ...
          hash[property] = mapping[:json_to_db].call(hash[property]) if mapping && hash.has_key?(property)
        end
      ...
      def map_db_types_to_json(schema, hash)
        ...
        schema['properties'].each do |property, definition|
          ...
          hash[property] = mapping[:db_to_json].call(hash[property]) if mapping && hash.has_key?(property)
        end
