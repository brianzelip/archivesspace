class Resource < Record
  ...
  def metadata
    ...
    unless json['lang_materials'].blank?
      md['inLanguage'] = json['lang_materials'].reject { |lang_material|
    ...
    md['holdingArchive'].delete_if { |_key, value| value.empty? }
    ...
    unless json['file_versions'].blank?
      json['file_versions'].each do |version|
        if version.dig('publish') != false && version['file_uri'].start_with?('http')
          dig_f['caption'] = json['html']['note']['note_text'] if json.dig('html', 'note', 'note_text')
  ...
  def parse_related_accessions
    ASUtils.wrap(raw['related_accession_uris']).collect { |uri|
      raw['_resolved_related_accession_uris'][uri].first if raw['_resolved_related_accession_uris'] && !raw['_resolved_related_accession_uris'][uri].nil?
  ...
  def parse_cite_string
    ...
    if cite.blank?
      ...
      cite += " #{repository_information['top']['name']}." unless repository_information['top']['name'].blank?
    else
