...
class PeriodicIndexer < IndexerCommon
  ...
  def start_worker_thread(queue, record_type)
    ...
    Thread.new do
      begin
        ...
        loop do
          id_subset = queue.poll(10_000, java.util.concurrent.TimeUnit::MILLISECONDS)
  ...
  def run_index_round
    ...
    # them.
    unless updated_repositories.empty?
    ...
    repositories.each_with_index do |repository, i|
      ...
      record_types.each do |type|
        ...
        workers = (0...@thread_count).map { |_thread_idx|
        ...
        begin
          ...
          id_set.each_slice(@records_per_thread) do |id_subset|
            ...
            until work_queue.offer(id_subset, 5000, java.util.concurrent.TimeUnit::MILLISECONDS)
  ...
  def handle_deletes(opts = {})
    ...
    loop do
      ...
      did_something = true unless deletes['results'].empty?
  ...
  def self.get_indexer(state = nil, name = 'Staff Indexer')
    indexer = new(AppConfig[:backend_url], state, name)
  end
