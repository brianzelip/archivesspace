class DigitalObjectsController < ApplicationController
  ...
  def show
    flash.keep unless flash.empty? # keep the notices so they display on the subsequent ajax call
  ...
  def edit
    ...
    if params[:inline]
      ...
      return redirect_to(action: :show, id: params[:id], inline: params[:inline]) if @digital_object.suppressed
  ...
  def add_children
    ...
    if params[:digital_record_children].blank? || params[:digital_record_children]['children'].blank?
      ...
      begin
        ...
        if params['validate_only'] == 'true'
          ...
          error_count = @exceptions.reject { |e| e.empty? }.length
          if error_count > 0
        ...
        flash.now[:error] = I18n.t('rde.messages.rows_with_errors', count: @exceptions.reject { |e| e.empty? }.length)
      end
  ...
  def fetch_tree
    ...
    prepare_tree_nodes(tree) do |node|
      ...
      node['children'] = true if node['has_children'] && node['children'].empty?
