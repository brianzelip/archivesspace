module SearchHelper
  ...
  def build_search_params(opts = {})
    ...
    sort = @search_data[:criteria]['sort'] if sort.nil? && !@search_data.nil? && @search_data.sorted?
    ...
    if params['advanced']
      ...
      params.keys.each do |param_key|
        ['op', 'f', 'v', 'dop', 't', 'top'].each do |adv_search_prefix|
          search_params[param_key] = params[param_key] if param_key =~ /^#{adv_search_prefix}\d+/
        end
      end
    end

    search_params.reject { |k, v| k.blank? || v.blank? }
  end
  ...
  def context_column_header_label
    @context_column_header || I18n.t('search_results.context')
  end
  ...
  def title_column_header_label
    @title_column_header || I18n.t('search_results.result_title')
  end

  def title_sort_label
    @title_column_header || I18n.t('search_sorting.title_sort')
  end
  ...
  def add_column(label, block, opts = {})
    ...
    @search_data.sort_fields << opts[:sort_by] if opts[:sortable] && opts[:sort_by]
  ...
  def add_browse_columns(model, enum_locales = {})
    (1..5).to_a.each do |n|
      ...
      next unless prop && prop != 'no_value'
  ...
  def get_ancestor_title(field)
    unless JSONModel::HTTP.get_json(field).nil?
  ...
  def context_separator(result)
    @separator = if result['ancestors'] || result['linked_instance_uris']
                   '>'
                 else
                   '<br />'.html_safe
                 end
  ...
  def context_ancestor(result)
    ancestors = if result['ancestors']
                  result['ancestors']
                elsif result['linked_instance_uris']
                  result['linked_instance_uris']
                elsif result['linked_record_uris']
                  result['linked_record_uris']
                elsif result['primary_type'] == 'top_container'
                  result['collection_uri_u_sstr']
                elsif result['primary_type'] == 'digital_object_component'
                  result['digital_object'].split
                else
                  ['']
                end
  end
