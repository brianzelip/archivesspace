...
class ClassificationTerm < Sequel::Model(:classification_term)
  ...
  def self.create_from_json(json, _opts = {})
    set_path_from_root(json)
    obj = super(json, title_sha1: Digest::SHA1.hexdigest(json.title))
    obj.reindex_children
    obj
  end

  def update_from_json(json, _opts = {}, apply_nested_records = true)
  ...
  def self.set_path_from_root(json)
    ...
    parent_id = json.parent ? parse_reference(json.parent['ref'], {})[:id] : nil
    ...
    root = Classification[parse_reference(json.classification['ref'], {})[:id]]
