...
Sequel.migration do
  up do
    warn('Adding range date to the list of date_type enums')
    ...
    warn('Changing all agent dates that are bulk_dates to ranges ')
    dates.exclude(Sequel.~(agent_person_id: nil) & Sequel.~(agent_family_id: nil) & Sequel.~(agent_corporate_entity_id: nil) & Sequel.~(agent_software_id: nil)).update(date_type_id: range)
  end
end
