...
module JSONModelPublishing
  ...
  def self._drop_unpublished(obj)
    if obj.is_a?(Array)
      obj.reject! { |item| _drop_unpublished(item) }
      obj.each do |value|
        _drop_unpublished(value)
      end
    elsif obj.is_a?(Hash)
      return true if obj.has_key?('publish') && !obj['publish']

      obj.reject! { |_k, v| _drop_unpublished(v) }
      _drop_unpublished(obj.values)
    end
      ...
      hash['notes'] = hash['notes'].map { |note|
        ...
        if note.has_key?('subnotes')
          note['subnotes'] = note['subnotes'].map { |subnote|
            subnote['content'] = MixedContentParser.parse(subnote['content'], '/', wrap_blocks: true) if subnote.has_key?('content')
