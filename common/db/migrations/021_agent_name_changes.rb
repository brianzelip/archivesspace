...
Sequel.migration do
  up do
    AGENT_TYPES = [:person, :family, :software, :corporate_entity].freeze
    ...
    NAME_TABLES = [:name_person, :name_corporate_entity, :name_family, :name_software].freeze

    NAME_TABLES.each do |name_type|
      self[name_type].each_by_page do |row|
        next unless row[:authority_id]

        authority_id = row[:authority_id]

        unless self[:name_authority_id].filter(authority_id: authority_id).empty?
          warn("WARNING: Authority ID #{authority_id} was not unique.")
          authority_id = "#{authority_id}_#{SecureRandom.hex}"
          warn("WARNING: Rewritten to #{authority_id}.")
        end
