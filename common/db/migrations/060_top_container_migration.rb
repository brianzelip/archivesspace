...
Sequel.migration do
  up do
    if AppConfig[:plugins].include?('container_management')
      warn '*' * 100
      warn "*\t\t You have the container_managment set in your AppConfig[:plugins] setting. We will not run the db migrations related to incorporating it into the ArchivesSpace core."
      warn '*' * 100
      break
    end
    ...
    create_enum('dimension_units', ['inches', 'feet', 'yards', 'millimeters', 'centimeters', 'meters']) if self[:enumeration].filter(name: 'dimension_units').count == 0
