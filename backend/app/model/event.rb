...
class Event < Sequel::Model(:event)
  ...
  def has_active_linked_records?
    related_records(:event_link).each do |linked_record|
      return true if linked_record.values.has_key?(:suppressed) && linked_record[:suppressed] == 0
    end
  ...
  def self.sequel_to_jsonmodel(objs, opts = {})
    ...
    jsons.each do |json|
      json['timestamp'] = json['timestamp'].iso8601 if json['timestamp']
    end
  ...
  def self.for_component_transfer(archival_object_uri, source_resource_uri, target_resource_uri)
    ...
    create_from_json(event, system_generated: true)
  end
  ...
  def self.for_archival_record_merge(target, victims)
    ...
    victims.each do |victim|
      ...
      identifier = if victim_json['identifier']
                     Identifiers.format(Identifiers.parse(victim_json['identifier']))
                   else
                     victim_json['digital_object_id']
                   end
