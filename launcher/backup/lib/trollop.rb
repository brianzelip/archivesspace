  ...
  class Parser
    ...
    def opt(name, desc = '', opts = {})
      ...
      type_from_default =
        ...
        when Date then :date
        when Array
          raise ArgumentError, "multiple argument type cannot be deduced from an empty array for '#{opts[:default][0].class.name}'" if opts[:default].empty?
      ...
      ## fill in :short
      opts[:short] = opts[:short].to_s unless opts[:short] == :none || !opts[:short]
    ...
    def parse(cmdline = ARGV)
      ...
      opt :version, 'Print version and exit' unless @specs[:version] || @long['version'] || !@version
