...
module ApplicationHelper
  ...
  def render_token(opts)
    ...
    html = "<div class='"
    html += 'token ' unless opts[:inside_token_editor]
    ...
    html += if opts[:icon_class]
              "<span class='icon-token #{opts[:icon_class]}'></span>"
            else
  ...
  def link_to_help(opts = {})
    return unless ArchivesSpaceHelp.enabled?
    return if opts.has_key?(:topic) && !ArchivesSpaceHelp.topic?(opts[:topic])
  ...
  def current_repo
    ...
    return @current_repo unless @current_repo.nil?
  ...
  def display_audit_info(hash, opts = {})
    ...
    if hash['create_time'] && hash['user_mtime']
      if fmt == 'wide'
  ...
  def has_permission_for_controller?(_session, name)
  ...
  def get_subject_icon_class(obj)
    if obj['_resolved']
      if obj['_resolved']['jsonmodel_type'] &&
        ...
        term_type = begin
                      obj['_resolved']['terms'][0]['term_type']
                    rescue StandardError
                      nil
                    end
