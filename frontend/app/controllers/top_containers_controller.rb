...
class TopContainersController < ApplicationController
  ...
  class MissingFilterException < RuntimeError; end
  ...
  def update_barcodes
    ...
    return render_aspace_partial partial: 'top_containers/bulk_operations/error_messages', locals: { exceptions: result, jsonmodel: 'top_container' }, status: 500 if response.code =~ /^4/
  ...
  def perform_search
    ...
    builder.and('collection_uri_u_sstr', params['collection_resource']['ref'], 'text', literal = true) if params['collection_resource']

    builder.and('collection_uri_u_sstr', params['collection_accession']['ref'], 'text', literal = true) if params['collection_accession']

    builder.and('container_profile_uri_u_sstr', params['container_profile']['ref'], 'text', literal = true) if params['container_profile']

    builder.and('location_uri_u_sstr', params['location']['ref'], 'text', literal = true) if params['location']
    ...
    builder.and('empty_u_sbool', (params['empty'] == 'yes'), 'boolean') unless params['empty'].blank?
    ...
    unless params['barcodes'].blank?
      ...
      builder.and(barcode_query) unless barcode_query.empty?
    end

    raise MissingFilterException if builder.empty? && params['q'].blank?
