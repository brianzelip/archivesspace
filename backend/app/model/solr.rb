  ...
  class Query
    ...
    def self.construct_advanced_query_string(advanced_query, use_literal = false)
      if advanced_query.has_key?('subqueries')
        ...
        clauses << '*:*' if advanced_query['subqueries'].all? { |subquery| subquery['negated'] }
    ...
    def initialize(query_string, _opts = {})
    ...
    def add_solr_params_from_config
      if AppConfig[:solr_params].any?
        AppConfig[:solr_params].each do |param, value|
          if value.is_a? Array
            value.each do |v|
              add_solr_param(param, v.respond_to?(:call) ? instance_eval(&v) : v)
            end
          else
            add_solr_param(param, value.respond_to?(:call) ? instance_eval(&value) : value)
          end
        end
      end
    end
    ...
    def set_repo_id(repo_id)
      add_solr_param(:fq, "repository:\"/repositories/#{repo_id}\" OR repository:global") if repo_id

      self
    end

    def set_root_record(root_record)
      add_solr_param(:fq, "(resource:\"#{root_record}\" OR digital_object:\"#{root_record}\")") if root_record
    ...
    def to_solr_url
      ...
      add_solr_param(:fq, 'publish:true') if @show_published_only
  ...
  def self.search(query)
    ...
    ASHTTP.start_uri(url) do |http|
      ...
      if solr_response.code == '200'
        ...
        result['highlighting'] = json['highlighting'] if json['highlighting']
