...
module DriverMixin
  ...
  def click_and_wait_until_element_gone(element)
    ...
    begin
      Selenium::Config.retries.times do |try|
        break unless element.displayed? || find_element_orig(*selector)
  ...
  def wait_until_gone(*selector)
    ...
    wait.until { !find_element_orig(*selector).displayed? }
    sleep 0.5
  end
  ...
  module WebDriver
    ...
    class Driver
      ...
      def find_paginated_element(*selectors)
        ...
        loop do
          begin
            ...
            begin
              click_and_wait_until_element_gone(find_element_orig(css: "a[title='Next']"))
            rescue Selenium::WebDriver::Error::NoSuchElementError
              puts 'Failed to turn the page!'
              raise Selenium::WebDriver::Error::NoSuchElementError, selectors.inspect
            end
          end
        end
      end
      ...
      def ensure_no_such_text(xpath, pattern, noError = false, noRetry = true)
        wait_for_ajax
        begin
          element = find_element(tag_name: 'body').find_element_with_text(xpath, pattern, noError, noRetry)
      ...
      def complete_4part_id(pattern, accession_id = nil)
        ...
        accession_id.each_with_index do |elt, i|
          clear_and_send_keys([:id, format(pattern, i)], elt)
        end
      ...
      def find_element_with_text(xpath, pattern, noError = false, noRetry = false)
        ...
        begin
          find_element(tag_name: 'body').find_element_with_text(xpath, pattern, noError, noRetry)
      ...
      def raise_javascript_errors
        errors = execute_script("return window.hasOwnProperty('TEST_ERRORS') ? TEST_ERRORS : []")
        raise "Javascript errors present: #{errors.inspect}" unless errors.empty?
      end
      ...
      def typeahead_and_select(token_input, value, try = 0)
        ...
        if token_input['value'] == value
          ...
          warn "Input did not have value #{value}, found value #{token_input['value']}. trying to enter input again.."
          typeahead_and_select(token_input, value, retries - 1)
        end
      end

      def open_rde_add_row_dropdown
        modal = find_element(id: 'rapidDataEntryModal')
        3.times do |_try|
          begin
    ...
    class Element
      ...
      def wait_for_class(className)
        try = 0
        until attribute('class').split(' ').include? className
          if try > Selenium::Config.retries
      ...
      def select_option(value)
        ...
        find_elements(tag_name: 'option').each do |option|
          begin
      ...
      def select_option_with_text(value)
        find_element(:xpath, "./*[contains( text(), '#{value.strip}' )]").click
      end
      ...
      def nearest_ancestor(xpath)
        find_element(xpath: './ancestor-or-self::' + xpath + '[1]')
      end
      ...
      def find_element_with_text(xpath, pattern, noError = false, noRetry = false)
        Selenium::Config.retries.times do |try|
          begin
            matches = find_elements(xpath: xpath)
        ...
        raise Selenium::WebDriver::Error::NoSuchElementError, "Could not find element for xpath: #{xpath} pattern: #{pattern}"
      end
