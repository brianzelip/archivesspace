class UsersController < ApplicationController
  ...
  def new
    if AppConfig[:allow_user_registration] || (session['user'] && user_can?('manage_users'))
  ...
  def edit
    ...
    if @user.is_system_user && !user_is_global_admin?
      flash[:error] = I18n.t('user._frontend.messages.access_denied', JSONModelI18nWrapper.new(user: @user))
      redirect_to(controller: :users, action: :index) && return
    end
  ...
  def update
    handle_crud(instance: :user,
                ...
                params_check: lambda { |obj, params|
                  if params['user']['password'] || params['user']['confirm_password']
                    obj.add_error('confirm_password', "entered value didn't match password") if params['user']['password'] != params['user']['confirm_password']
                  end
                },
                on_invalid: lambda {
    ...
    handle_crud(instance: :user,
                params_check: lambda { |obj, params|
                  ['password', 'confirm_password'].each do |field|
                    obj.add_error(field, "Can't be empty") if params['user'][field].blank?
                  end
                  obj.add_error('confirm_password', "entered value didn't match password") if params['user']['password'] != params['user']['confirm_password']
