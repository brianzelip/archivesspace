class EACSerializer < ASpaceExport::Serializer
  ...
  def _control(json, xml)
    xml.control {
      ...
      xml.maintenanceAgency {
        ...
        xml.agencyCode json.maintenanceAgency.agencyCode if json.maintenanceAgency.agencyCode
      }
  ...
  def _cpfdesc(json, xml)
    xml.cpfDescription {
      ...
      xml.description {
        ...
        json.notes.select { |n| n['jsonmodel_type'] == 'note_bioghist' }.each do |n|
      ...
      xml.relations {
        ...
        json.external_documents.each do |document|
          next unless document['location'] =~ %r{^http[s]?://.+}
  ...
  def _build_date_ranges(date, xml)
    xml.dateRange date['expression'] if date['expression']
