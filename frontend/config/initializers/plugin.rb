...
module Plugins
  def self.init
    ...
    puts "Found system menu items for plug-ins: #{system_menu_items.inspect}" unless @config[:system_menu_items].empty?

    puts "Found repository menu items for plug-ins: #{repository_menu_items.inspect}" unless @config[:repository_menu_items].empty?
  end
