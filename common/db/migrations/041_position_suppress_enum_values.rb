...
Sequel.migration do
  up do
    auto_increment_opts = database_type == :derby ? { default: 0 } : { auto_increment: true, default: 0 }
    ...
    self[:enumeration_value].order(:value).to_hash_groups(:enumeration_id).each_pair do |_enum, rows|
