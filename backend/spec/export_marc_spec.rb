...
describe 'MARC Export' do
  ...
  describe 'root node content' do
    ...
    it 'root node should have xmlns:xsi defined' do
      expect(@xml).to match(%r{<collection.*xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"})
    end

    it 'root node should have xsi:schemaLocation defined' do
      expect(@xml).to match(%r{<collection.*xsi:schemaLocation="http://www.loc.gov/MARC21/slim http://www.loc.gov/standards/marcxml/schema/MARC21slim.xsd"})
    end
  end
  ...
  describe 'datafield 3xx mapping' do
    before(:each) do
      @notes = ['arrangement', 'fileplan'].map { |type|
  ...
  describe 'agents: include unpublished flag' do
    ...
    it 'should create elements for unpublished agents if include_unpublished is true' do
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '100']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '610']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '600']").empty?).to be_truthy
    end
  end
  ...
  describe 'linked agent mappings' do
    ...
    it "maps the first creator to df[@tag='100'] when it's a person" do
      ...
      name_string = ['primary_', 'rest_of_'].map { |p| name["#{p}name"] }.reject { |n| n.nil? || n.empty? }.join(name['name_order'] == 'direct' ? ' ' : ', ')
      ...
      expect(df.at("subfield[@code='c']")).to have_inner_text(/#{['prefix', 'title', 'suffix'].map { |p| name[p] }.compact.join(', ')}/)
    ...
    it 'should add required punctuation to 100 tag agent-person subfields' do
      ...
      expect(q_text[-1]).to eq(',') unless c_text.nil? || c_text.empty?

      expect(c_text[-1]).to eq(',') unless d_text.nil? || d_text.empty?
    ...
    it "maps the first creator to df[@tag='110'] when it's a corp" do
      ...
      expect(subfield_b).to have_inner_text(/#{name['subordinate_name_1']}/) unless subfield_b.nil?
    ...
    it "maps subject to 600 when it's a person" do
      ...
      expect(df.at("subfield[@code='2']")).to have_inner_text(/#{name['source']}/) if ind2 == '7'
    end
  ...
  describe 'note mappings' do
    ...
    it 'maps notes of type (odd|dimensions|physdesc|materialspec|physloc|phystech|physfacet|processinfo|separatedmaterial) to df 500, sf a' do
      ...
      types = ['odd', 'dimensions', 'physdesc', 'materialspec', 'physloc', 'phystech', 'physfacet', 'processinfo', 'separatedmaterial']
      notes = @resource.notes.select { |n| types.include?(n['type']) }
      notes.each do |note|
        prefix = case note['type']
                 when 'odd' then nil
                 when 'dimensions' then 'Dimensions'
                 when 'physdesc' then 'Physical Description note'
                 when 'materialspec' then 'Material Specific Details'
                 when 'physloc' then 'Location of resource'
                 when 'phystech' then 'Physical Characteristics / Technical Requirements'
                 when 'physfacet' then 'Physical Facet'
                 when 'processinfo' then 'Processing Information'
                 when 'separatedmaterial' then 'Materials Separated from the Resource'
        end
    ...
    it "maps public notes of type 'custodhist' to df 561 ('1', ' '), sf a" do
      note_test(@resource, @marc, ['custodhist'], ['561', '1', ' '], 'a', 'publish' => true)
    end

    it "maps private notes of type 'custodhist' to df 561 ('0', ' '), sf a" do
      note_test(@resource, @marc, ['custodhist'], ['561', '0', ' '], 'a', 'publish' => false)
    end

    it "maps public notes of type 'appraisal' to df 583 ('1', ' '), sf a" do
      note_test(@resource, @marc, ['appraisal'], ['583', '1', ' '], 'a', 'publish' => true)
    end

    it "maps private notes of type 'appraisal' to df 583 ('0', ' '), sf a" do
      note_test(@resource, @marc, ['appraisal'], ['583', '0', ' '], 'a', 'publish' => false)
    end

    it "maps notes of type 'accruals' to df 584 (' ', ' '), sf a" do
      note_test(@resource, @marc, ['accruals'], ['584', ' ', ' '], 'a')
    end
  ...
  describe 'notes: include unpublished flag' do
    ...
    it 'should create elements for unpublished notes if include_unpublished is true' do
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '506']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '524']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '535']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '540']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '541']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '544']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '545']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '561']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '583']").empty?).to be_truthy
      expect(!@marc_unpub_incl.xpath("//marc:datafield[@tag = '584']").empty?).to be_truthy
    end
  end
