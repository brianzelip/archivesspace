...
class TopContainer < Sequel::Model(:top_container)
  ...
  SERIES_LEVELS = ['series'].freeze
  OTHERLEVEL_SERIES_LEVELS = ['accession'].freeze
  ...
  def validate
    ...
    errors.add(:barcode, 'Length must be within the range set in configuration') unless check.valid?(self[:barcode])
  ...
  def calculate_collections
    ...
    resource_ids = []
    resource_ids += self.class.resource_ids_linked_directly(id)

    # Resource linked via AO
    resource_ids += self.class.resource_ids_linked_via_ao(id)
  ...
  def self.sequel_to_jsonmodel(objs, opts = {})
    ...
    jsons.zip(objs).each do |json, obj|
      ...
      json['exported_to_ils'] = json['exported_to_ils'].getlocal.iso8601 if json['exported_to_ils']
    end
  ...
  def self.search_stream(params, repo_id, &block)
    ...
    max_results = AppConfig.has_key?(:max_top_container_results) ? AppConfig[:max_top_container_results] : 10_000
    ...
    query.set_filter(AdvancedQueryBuilder.from_json_filter_terms(params[:filter_term])) if params[:filter_term]
    ...
    ASHTTP.start_uri(url) do |http|
      http.request(req, nil) do |response|
        raise response.body if response.code =~ /^4/
  ...
  def self.batch_update(ids, fields)
    out = {}
    begin
      n = filter(id: ids).update(fields.merge(system_mtime: Time.now, user_mtime: Time.now))
  ...
  def self.bulk_update_container_profile(ids, container_profile_uri)
    ...
    begin
      ...
      unless container_profile_uri.empty?
        ...
        raise "Container profile not found: #{container_profile_uri}" unless container_profile
  ...
  def self.bulk_update_location(ids, location_uri)
    ...
    begin
      ...
      raise "Location not found: #{location_uri}" unless location
  ...
  def self.for_barcode(barcode)
    TopContainer[barcode: barcode, repo_id: active_repository]
  end

  def self.for_indicator(indicator)
    TopContainer[indicator: indicator, repo_id: active_repository]
  end
