module HandleFaceting
  ...
  def fetch_only_facets(query, facets_array, include_zero)
    ...
    criteria['facet.mincount'] = 1 unless include_zero
    data = archivesspace.search(query, 1, criteria) || {}
    faceting = {}
    faceting = data['facets']['facet_fields'] if !data['facets'].blank? && !data['facets']['facet_fields'].blank?
   end
  ...
  def get_pretty_facet_value(k, v)
    ...
    elsif ['repository', 'used_within_published_repository'].include?(k)
