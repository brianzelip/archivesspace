...
module JSONModel
          ...
          begin
            ...
            notifications.each do |notification|
              @@notification_handlers.each do |handler|
                next unless handler[:code].nil? || (handler[:code] == notification['code'])

                begin
                  handler[:block].call(notification['code'], notification['params'])
                rescue StandardError
                  warn("ERROR: Failed to handle notification #{notification.inspect}: #{$!}")
                end
              end
            end
  ...
  module HTTP
    ...
    def self.post_form(uri, params = {}, encoding = :x_www_form_urlencoded)
      ...
      req = if encoding == :x_www_form_urlencoded
              form_urlencoded(url.request_uri, params)
            elsif encoding == :multipart_form_data
              multipart_request(url.request_uri, params)
            else
    ...
    def self.stream(uri, params = {}, &block)
      ...
      req['X-ArchivesSpace-Priority'] = 'high' if high_priority?
    ...
    def self.get_json(uri, params = {})
      ...
      ASUtils.json_parse(response.body) if response.is_a?(Net::HTTPSuccess) || response.code == '200'
    end
    ...
    def self.do_http_request(url, req, &block)
      ...
      JSONModel.handle_error(ASUtils.json_parse(response.body)) if response.code =~ /^4/
  ...
  module Client
    ...
    def save(opts = {}, whole_body = false)
      ...
      response = JSONModel::HTTP.post_json(self.class.my_url(id, opts),
                                           to_json)
      ...
      if response.code == '200'
        ...
        raise ConflictException, err['error']
        ...
        raise Exception, "Unknown response: #{response.body} (code: #{response.code})"
      end
    end
    ...
    def refetch
      ...
      obj = (instance_data.has_key? :find_opts) ?
                self.class.find(id, instance_data[:find_opts]) : self.class.find(id)

      reset_from(obj) unless obj.nil?
    end

    def delete
      response = JSONModel::HTTP.delete_request(self.class.my_url(id))
    ...
    def set_suppressed(val)
      response = JSONModel::HTTP.post_form("#{uri}/suppressed", suppressed: val)

      if response.code == '403'
        raise AccessDeniedException, 'Permission denied when setting suppression status'
    ...
    module ClassMethods
      def self.extended(base)
        class << base
          ...
          def substitute_parameters(uri, opts = {})
            ...
            opts = { 'repo_id' => JSONModel.repository }.merge(opts) if JSONModel.repository
      ...
      def my_url(id = nil, opts = {})
        uri, remaining_opts = uri_and_remaining_options_for(id, opts)
        ...
        url.query = URI.encode_www_form(remaining_opts) unless remaining_opts.empty?
      ...
      def find(id, opts = {})
        ...
        if response.code == '200'
          obj = new(ASUtils.json_parse(response.body))
      ...
      def all(params = {}, opts = {})
        ...
        if response.code == '200'
          ...
          if json_list.is_a?(Hash)
            json_list['results'] = json_list['results'].map { |h| new(h) }
          else
            json_list = json_list.map { |h| new(h) }
          end
