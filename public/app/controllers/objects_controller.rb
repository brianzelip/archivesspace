class ObjectsController < ApplicationController
  ...
  DEFAULT_OBJ_FACET_TYPES = ['repository', 'primary_type', 'subjects', 'published_agents'].freeze
  ...
  def index
    ...
    begin
      ...
      redirect_back(fallback_location: '/objects') && return
    end
  ...
  def search
    ...
    page = Integer(params.fetch(:page, '1'))
    begin
      set_up_and_run_search(['digital_object', 'archival_object'], DEFAULT_OBJ_FACET_TYPES, DEFAULT_OBJ_SEARCH_OPTS, params)
    rescue NoResultsError
      flash[:error] = I18n.t('search_results.no_results')
      redirect_back(fallback_location: '/') && return
    rescue Exception => e
  ...
  def show
    ...
    if params[:obj_type] == 'archival_objects'
      url = uri += '#pui' unless uri.ends_with?('#pui')
    end
    uri = uri.sub("\#pui", '')
    @criteria = {}
