...
ASUtils.wrap(ASUtils.find_local_directories).map { |local_dir| File.join(local_dir, 'frontend', 'locales') }.select { |dir| Dir.exist?(dir) }.each do |locales_override_directory|
