...
module ManipulateNode
  ...
  def process_mixed_content(in_txt, opts = {})
    ...
    return txt unless needs_nokogiri
  ...
  def strip_mixed_content(in_text)
    ...
    # Don't fire up nokogiri if there's no mixed content to parse
    return in_text unless in_text.include?('<')
  ...
  def node_check(el)
    ...
    if newnode.key? 'render'
      ...
      newnode.name = if !el.ancestors('thead').empty?
                       'th'
                     else
                       'td'
                     end
      ...
      unless newnode.name.match(/^(p|ul|li|br|h5)$/)
        ...
        clss = newnode['class'] || ''
        role = newnode['role'] || ''
        newnode.remove_attribute('role') unless role.blank?
  ...
  def process_render(node)
    ...
    clss = node['class'] || ''
    clss = "#{name} #{clss}"
    ['altrender', 'bold', 'single', 'italic', 'smcaps', 'nonproport'].each do |w|
      clss = "#{w} #{clss}" if node['render'].include?(w)
    end
    node['class'] = clss.strip unless clss.blank?
