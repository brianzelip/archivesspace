...
module ASpaceExport
  ...
  def self.serializer(name, &block)
    if block_given?
      ...
      raise SerializerNotFoundError, "Can't find a serializer for: #{name}"
    end
  end
  ...
  def self.model(name, &block)
    if block_given?
      ...
      raise ExportModelNotFoundError, "Can't find a model for: #{name}"
    end
  end

  def self.get_serializer_for(model, opts)
    key = opts[:serializer] || model.class.instance_variable_get(:@model_for)
  ...
  class ExportModel
    ...
    def apply_map(obj, map, opts = {})
      ...
      map.each do |as_field, handler|
        ...
        fieldable = [as_field].flatten.select { |asf| obj.respond_to?(asf) }

        next if fieldable.empty?

        handler_args = fieldable.map { |f| obj.send(f) }
        [handler].flatten.each { |h| send(h, *handler_args) }
      end
    end
  end
