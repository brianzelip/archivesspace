...
module ASModel
  ...
  module CRUD
    ...
    def apply_nested_records(json, new_record = false)
      remove_nested_records unless new_record
      ...
      self.class.nested_records.each do |nested_record|
        ...
        add_record_method = if nested_record[:association][:type] === :one_to_one
                              nested_record[:association][:name].to_s
                            elsif nested_record[:association][:type] === :many_to_one
                              "#{nested_record[:association][:name].to_s.singularize}="
                            else
                              "add_#{nested_record[:association][:name].to_s.singularize}"
                            end
        ...
        updated_records = []
        (records || []).each_with_index do |json_or_uri, i|
          ...
          begin
            ...
            if json_or_uri.is_a? String
              ...
              if model.respond_to? :ensure_exists
                ...
                if nested_record[:association][:key]
                  extra_opts[nested_record[:association][:key]] = id
            ...
            send(add_record_method, db_record) if db_record && needs_linking
    ...
    def remove_nested_records
      self.class.nested_records.each do |nested_record_defn|
        if [:one_to_one, :one_to_many].include?(nested_record_defn[:association][:type])
          ...
          Array(send(nested_record_defn[:association][:name])).each do |nested_record|
            nested_record.delete
          end
        elsif nested_record_defn[:association][:type] === :many_to_many
          # Just remove the links
          send("remove_all_#{nested_record_defn[:association][:name]}".intern)
        elsif nested_record_defn[:association][:type] === :many_to_one
          # Just remove the link
          send("#{nested_record_defn[:association][:name].intern}=", nil)
        end
      end
    end

    def update_from_json(json, extra_values = {}, apply_nested_records = true)
      if values.has_key?(:suppressed)
        raise ReadOnlyException, "Can't update an object that has been suppressed" if self[:suppressed] == 1
      ...
      raise ConflictException, 'You must provide a lock_version in your request' if updated.has_key?('lock_version') && !updated['lock_version']

      self.class.strict_param_setting = false

      update(self.class.prepare_for_db(json.class, updated)
                  .merge(user_mtime: Time.now,
                         last_modified_by: RequestContext.get(:current_username)))

      self.apply_nested_records(json) if apply_nested_records

      self.class.fire_update(json, self)

      create_ark_name if AppConfig[:arks_enabled] && !ArkName.ark_name_exists?(id, self.class)
    ...
    def delete
      ...
      loop do
        ...
        object_graph.each do |model, ids_to_delete|
          ...
          next unless model.my_jsonmodel(true)

          ids_to_delete.each do |id|
            deleted_model = model.my_jsonmodel(true)

            # ArkNames don't have URIs, so they are deleted above
            deleted_uri = deleted_model.uri_for(id, repo_id: model.active_repository) unless model == ArkName

            deleted_uris << deleted_uri if deleted_uri
          end
        end
        ...
        raise ConflictException, "Record deletion failed: #{last_error}"
      end
    ...
    def create_ark_name
      ArkName.create_from_resource(self) if self.class == Resource

      ArkName.create_from_archival_object(self) if self.class == ArchivalObject
    end

    def delete_ark_name
      ArkName.first(resource_id: id)&.delete if self.class == Resource

      ArkName.first(archival_object_id: id)&.delete if self.class == ArchivalObject
    end
    ...
    module ClassMethods
      ...
      def create_from_json(json, extra_values = {})
        ...
        values['repo_id'] = active_repository if model_scope == :repository && !values.has_key?('repo_id')

        values['created_by'] = RequestContext.get(:current_username)

        obj = create(prepare_for_db(json.class,
      ...
      def fire_update(_json, sequel_obj)
        if high_priority?
          model = self
      ...
      def def_nested_record(opts)
        opts[:association] = association_reflection(opts[:corresponding_to_association])
        opts[:jsonmodel] = opts[:contains_records_of_type]
        opts[:json_property] = opts[:the_property]

        opts[:is_array] = true unless opts.has_key?(:is_array)
      ...
      def get_or_die(id)
        obj = if model_scope == :repository
                this_repo[id: id]
              else
                self[id]
              end

        obj || raise(NotFoundException, "#{self} not found")
      end
      ...
      def corresponds_to(jsonmodel)
        ...
        @jsonmodel.schema['properties'].each do |prop, defn|
          enums << { property: prop, uses_enum: [defn['dynamic_enum']] } if defn['dynamic_enum']
        end
      ...
      def my_jsonmodel(ok_if_missing = false)
        @jsonmodel || (ok_if_missing ? nil : raise("No corresponding JSONModel set for model #{inspect}"))
      end

      def sequel_to_jsonmodel(objs, _opts = {})
      ...
      def to_jsonmodel(obj, opts = {})
        ...
        if is_id_query || is_string_query
          ...
          raise NotFoundException, "#{self} not found" unless obj
      ...
      def handle_delete(ids_to_delete)
        filter(id: ids_to_delete).delete
      end

      def update_mtime_for_repo_id(repo_id)
        if model_scope == :repository
          dataset.filter(repo_id: repo_id).update(system_mtime: Time.now) if dataset.columns.include? :repo_id
        end
      end
      ...
      def update_mtime_for_ids(ids)
        ...
        ids.each_slice(50) do |subset|
          dataset.filter(id: subset).update(system_mtime: now)
        end
      end
