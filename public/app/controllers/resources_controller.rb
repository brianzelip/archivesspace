class ResourcesController < ApplicationController
  ...
  DEFAULT_RES_FACET_TYPES = ['primary_type', 'subjects', 'published_agents'].freeze
  ...
  DEFAULT_RES_TYPES = ['pui_archival_object', 'pui_digital_object', 'agent', 'subject'].freeze
  ...
  def index
    ...
    facet_types.unshift('repository') unless @repo_id
    begin
    ...
    @sort_opts.unshift(all_sorts['relevance']) if params[:q].size > 1 || params[:q][0] != '*'
    @result_props = {
      no_res: true
    }
    @no_statement = true
  ...
  def search
    ...
    if @results['total_hits'].blank? || @results['total_hits'] == 0
      ...
      title = ''
      title =  strip_mixed_content(@results['results'][0]['_resolved_resource']['json']['title']) if @results['results'][0]&.dig('_resolved_resource', 'json')

      @context = []
      @context.push(uri: "/repositories/#{repo_id}",
                    crumb: get_pretty_facet_value('repository', "/repositories/#{repo_id}"))
      @context.push(uri: "#{res_id}", crumb: title) unless title.blank?
      if @results['total_hits'] > 1
        @search[:dates_within] = true if params.fetch(:filter_from_year, '').blank? && params.fetch(:filter_to_year, '').blank?
        @search[:text_within] = true
      end
  ...
  CONTAINER_QUERY = 'types:pui AND types:pui_container'.freeze
