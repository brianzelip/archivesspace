...
class EAD3Serializer < EADSerializer
  ...
  def structure_children(content, parent_name = nil)
    ...
    p_wrap_invalid_children = lambda do |text|
      ...
      if valid_children
        ...
        fragment.element_children.each do |e|
          new_text << if valid_children.include?(e.name)
                        e.to_s
                      else
                        "<p>#{e}</p>"
                      end
    ...
    if !has_unwrapped_text?(content)
      ...
      blocks.each do |b|
        new_content << if has_unwrapped_text?(b)
                         p_wrap.call(b)
                       else
                         p_wrap_invalid_children.call(b)
                       end
      end
      content = new_content
    end
  ...
  def sanitize_mixed_content(content, context, fragments, allow_p = false)
    ...
    content = if allow_p
                structure_children(content, context.parent.name)
              else
                strip_p(content)
              end
  ...
  def convert_ead2002_markup(content)
    apply_changes = lambda do |fn, fragment|
      fragment.element_children.each do |e|
        fn.call(e)
        e.element_children.each { |ec| fn.call(ec) } unless e.element_children.empty?
      end
      fragment
    end

    strip_attribute_namespace_prefixes = lambda do |e|
      e.attributes.each do |_k, a|
    ...
    convert_attribute_names = lambda do |e|
      e.attributes.each do |_k, a|
        ...
        a.name = 'identifier' if a.name == 'authfilenumber'
      end
    end
    ...
    convert_list_attribute_values = lambda do |e|
      if e.name == 'list'
        ...
        e['numeration'] = list_numeration_value(e['numeration']) if e['numeration']
      end
    end
    ...
    downcase_closed_list_attribute_values = lambda do |e|
      e.attributes.each do |_k, a|
        e[a.name] = a.value.downcase if closed_list_attributes.include? a.name
      end
    end
    ...
    strip_invalid_children_of_mixed_elements = lambda do |e|
      ...
      unless children.empty?
        if (valid_children = valid_children_of_mixed_elements(e.name))
          children.each do |el|
            el.replace(el.inner_text.gsub(/\s+/, ' ')) if !valid_children.include?(el.name) && el.inner_text
          end
        end
      end
    end
    ...
    strip_text_content = lambda do |e|
      e.content = e.inner_text.strip if e.element_children.empty? && e.inner_text
    end
    ...
    process_fragment = lambda do |f|
      apply_changes.call(strip_attribute_namespace_prefixes, f)
      apply_changes.call(convert_extref, f)
      apply_changes.call(convert_attribute_names, f)
      apply_changes.call(convert_list_attribute_values, f)
      apply_changes.call(convert_type_to_localtype, f)
      apply_changes.call(wrap_access_terms_in_part, f)
      apply_changes.call(downcase_closed_list_attribute_values, f)
      apply_changes.call(strip_invalid_children_of_mixed_elements, f)
      apply_changes.call(strip_text_content, f)

      f.element_children.each do |e|
        process_fragment.call(e)
      end
    end

    process_fragment.call(fragment)
  ...
  def stream(data)
    ...
    builder = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
      begin
        ...
        ead_attributes['audience'] = 'internal' if data.publish === false
        ...
        xml.ead(ead_attributes) {
          ...
          atts.reject! { |_k, v| v.nil? }

          xml.archdesc(atts) {
            xml.did {
              xml.unittitle { sanitize_mixed_content(data.title, xml, @fragments) } unless data.title.nil?
            ...
            xml.dsc {
              data.children_indexes.each do |i|
                xml.text(@stream_handler.buffer { |xml, new_fragments|
  ...
  def serialize_control(data, xml, fragments)
    ...
    }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }
    ...
    xml.control(control_atts) {
      ...
      xml.filedesc {
        ...
        xml.publicationstmt {
          ...
          xml.date { sanitize_mixed_content(data.finding_aid_date, xml, fragments) } if data.finding_aid_date
      ...
      xml.maintenancehistory {
        ...
        export_rs = @include_unpublished ? data.revision_statements : data.revision_statements.select { |rs| rs['publish'] }
        unless export_rs.empty?
  ...
  def serialize_extents(obj, xml, fragments)
    if obj.extents.length
      obj.extents.each do |e|
        ...
        physdescstructured_atts[:audience] = 'internal' if e['publish'] === false
  ...
  def serialize_dates(obj, xml, fragments)
    add_unitdate = proc do |value, context, fragments, atts = {}|
    ...
    obj.dates.each do |date|
      ...
      date_atts = {
        certainty: date['certainty'] || nil,
        era: date['era'] || nil,
        calendar: date['calendar'] || nil,
      ...
      date_atts.delete_if { |_k, v| v.nil? }
      ...
      if date['begin'] || date['end']
        ...
        add_unitdate.call(date['expression'], xml, fragments, date_atts) if date['begin'] && date['end'] && date['expression']
  ...
  def strip_invalid_children_from_note_content(content, parent_element_name)
    ...
    unless children.empty?
      if valid_children = valid_children_of_mixed_elements(parent_element_name)
        children.each do |e|
          e.replace(e.inner_text.gsub(/\s+/, ' ')) if !valid_children.include?(e.name) && e.inner_text
        end
      end
    end
  ...
  def serialize_did_notes(data, xml, fragments)
    data.notes.each do |note|
      ...
      atts.delete_if { |_k, v| v.nil? || v.empty? || v == 'null' }

      append_note_content = proc do |note, context, fragments, parent_element_name|
      ...
      when 'dimensions', 'physfacet'
        ...
        xml.physdesc(atts) {
          append_note_content.call(note, xml, fragments, 'physdesc')
        }
      when 'physdesc'
        atts[:label] = note['label'] if note['label']
        xml.send(note['type'], atts) {
          append_note_content.call(note, xml, fragments, note['type'])
        }
      when 'langmaterial'
        xml.langmaterial(atts) {
          xml.language {
            append_note_content.call(note, xml, fragments, 'language')
          }
        }
      else
        xml.send(note['type'], atts) {
  ...
  def serialize_note_content(note, xml, fragments)
    ...
    head_text = note['label'] || I18n.t("enumerations._note_types.#{note['type']}", default: note['type'])
    ...
    xml.send(note['type'], atts) {
      ...
      serialize_subnotes(note['subnotes'], xml, fragments, ASpaceExport::Utils.include_p?(note['type'])) if note['subnotes']
    }
  end
  ...
  def serialize_origination(data, xml, fragments)
    data.creators_and_sources&.each do |link|
      ...
      role = link['role'] == 'creator' ? link['role'].capitalize : link['role']
  ...
  def extract_head_text(content, backup = '')
    content ||= ''
    match = content.strip.match(%r{<head( [^<>]+)?>(.+?)</head>})
  ...
  def serialize_child(data, xml, fragments, c_depth = 1)
    ...
    atts[:audience] = 'internal' if data.publish === false
          ...
          if ark_url
            ...
            xml.unitid {
              xml.ref ({ 'href' => ark_url,
                         'actuate' => 'onload',
                         'show' => 'new' }) { xml.text 'Archival Resource Key' }
            }
          end
        end

        xml.unitid data.component_id if !data.component_id.nil? && !data.component_id.empty?
  ...
  def serialize_subnotes(subnotes, xml, fragments, include_p = true)
    subnotes.each do |sn|
      ...
      when 'note_orderedlist'
        atts = { listtype: 'ordered', numeration: sn['enumeration'] }.reject { |_k, v| v.nil? || v.empty? || v == 'null' }.merge(audatt)
  ...
  def serialize_container(inst, xml, fragments)
    ...
    atts[:containerid] = "#{top['barcode']}" if top['barcode']
  ...
  def serialize_digital_object(digital_object, xml, fragments)
    ...
    if date['expression']
      ...
      content << date['begin']
      content << "-#{date['end']}" if date['end'] != date['begin']
    end
  ...
  def serialize_bibliographies(data, xml, fragments)
    data.bibliographies.each do |note|
      ...
      content = ASpaceExport::Utils.extract_note_text(note, @include_unpublished)
      note_type = note['type'] || 'bibliography'
      head_text = note['label'] || I18n.t("enumerations._note_types.#{note_type}", default: note_type)
