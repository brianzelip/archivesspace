...
module AgentManager
  ...
  def self.model_for(type)
    registered_agents.each do |agent_type|
  ...
  module Mixin
    ...
    def update_from_json(json, opts = {}, _apply_nested_records = true)
    ...
    def before_save
      self.agent_sha1 = SecureRandom.hex if agent_sha1.nil?
    ...
    def linked_agent_roles
      ...
      BackendEnumSource.values_for_ids(role_ids).values.select { |v| valid_enum.include?(v) }
    end
    ...
    module ClassMethods
      ...
      def ensure_authorized_name(json)
        json['names'][0]['authorized'] = true if !Array(json['names']).empty? && json['names'].none? { |name| name['authorized'] }
      end
      ...
      def ensure_exists(json, _referrer)
        DB.attempt {
          ensure_authorized_name(json)
      ...
      def create_from_json(json, opts = {})
        ...
        combine_unauthorized_names(json)
      ...
      def assemble_hash_fields(json)
        ...
        json.dates_of_existence.each do |date|
          fields << hash_chunk(JSONModel(:date).from_hash(date),
                               ['date_type', 'label', 'certainty', 'expression', 'begin', 'end', 'era', 'calendar'])
        end

        json.agent_contacts.each do |contact|
          fields << hash_chunk(JSONModel(:agent_contact).from_hash(contact),
                               ['name', 'salutation', 'telephone', 'address_1', 'address_2', 'address_3', 'city', 'region', 'country', 'post_code', 'email', 'email_signature', 'note'])
        end
      ...
      def register_agent_type(opts)
        ...
        one_to_many my_agent_type[:name_type]

        def_nested_record(the_property: :names,
