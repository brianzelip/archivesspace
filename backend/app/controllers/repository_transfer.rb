class ArchivesSpaceService < Sinatra::Base
  ...
  do
    ...
    RequestContext.open(repo_id: target_id) do
      raise AccessDeniedException.new(target_repo: ['Permission denied']) unless current_user.can?(:transfer_repository)
    end
  ...
  def handle_transfer(model, id, perm)
    ...
    raise BadParamsException.new(target_repo: ['Repository not found']) unless target

    RequestContext.open(repo_id: target.id) do
      raise AccessDeniedException.new(target_repo: ['Permission denied']) unless current_user.can?(perm)
    end
