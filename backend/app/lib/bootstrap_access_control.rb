class ArchivesSpaceService
  ...
  def self.create_hidden_system_user(username, name, password)
    create_system_user(username, name, password, true)
  end
  ...
  def self.set_up_base_permissions
    unless Repository[repo_code: Repository.GLOBAL]
    ...
    create_system_user(User.ADMIN_USERNAME, 'Administrator', AppConfig[:default_admin_password])
    create_group(Group.ADMIN_GROUP_CODE, 'Administrators', [User.ADMIN_USERNAME], [])
  ...
  def self.create_search_user
    create_hidden_system_user(User.SEARCH_USERNAME, 'Search Indexer', AppConfig[:search_user_secret])
    DBAuth.set_password(User.SEARCH_USERNAME, AppConfig[:search_user_secret])
    create_group(Group.SEARCHINDEX_GROUP_CODE, 'Search index', [User.SEARCH_USERNAME],
                 ['view_repository', 'view_suppressed', 'view_all_records', 'index_system'])
  end

  def self.create_public_user
    create_hidden_system_user(User.PUBLIC_USERNAME, 'Public Interface Anonymous', AppConfig[:search_user_secret])
    DBAuth.set_password(User.PUBLIC_USERNAME, AppConfig[:public_user_secret])
    create_group(Group.PUBLIC_GROUP_CODE, 'Public Anonymous', [User.PUBLIC_USERNAME],
                 ['view_repository', 'view_all_records'])
  end

  def self.create_staff_user
    create_hidden_system_user(User.STAFF_USERNAME, 'Staff System User', AppConfig[:search_user_secret])
    DBAuth.set_password(User.STAFF_USERNAME, AppConfig[:staff_user_secret])
    create_group(Group.STAFF_GROUP_CODE, 'Staff System Group', [User.STAFF_USERNAME],
                 ['mediate_edits'])
  end
