class AgentsController < ApplicationController
  ...
  def new
    ...
    @agent.names = [@name_type.new(authorized: true, is_display_name: true)._always_valid!] if @agent.names.empty?
  ...
  def update
    handle_crud(instance: :agent,
                ...
                on_invalid: lambda {
                  @agent.names = [@name_type.new._always_valid!] if @agent.names.empty?
  ...
  def assign_types
    return unless params.has_key? 'agent_type'
