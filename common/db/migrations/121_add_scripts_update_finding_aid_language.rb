  ...
  up do
    ...
    def migrate_language_notes(dataset)
      dataset.each do |row|
        ...
        parsed = begin
                   Nokogiri::XML::DocumentFragment.parse(note)
                 rescue StandardError
                   nil
                 end
        language = begin
                     parsed.xpath('.//language').attr('langcode')
                   rescue StandardError
                     nil
                   end
        unless language.nil?
        ...
        script = begin
                   parsed.xpath('.//language').attr('scriptcode')
                 rescue StandardError
                   nil
                 end
        unless script.nil?
        ...
        next unless note.match(%r{(<language langcode="[a-z]+" scriptcode="[A-z]+">(.*)</language>)|(<language langcode="[a-z]+">(.*)</language>)|(<language langcode="[a-z]+"/>)})

        content = note.sub(%r{(<language langcode="[a-z]+" scriptcode="[A-z]+">(.*)</language>)|(<language langcode="[a-z]+">(.*)</language>)|(<language langcode="[a-z]+"/>)}, '\\2\\4')
